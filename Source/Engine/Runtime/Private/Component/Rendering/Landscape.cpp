...
// 				m_HeightMapCS->SetHeightMap(HeightMap);
...
// 	Transform()->Binding();
//
// 	GetMaterial(0)->SetScalarParam(INT_0, FaceX); // 지형의 면 개수
// 	GetMaterial(0)->SetScalarParam(INT_1, FaceZ); // 지형의 면 개수
// 	GetMaterial(0)->SetScalarParam(INT_2, static_cast<int>(m_Mode)); // 지형 모드
//
// 	GetMaterial(0)->SetTexParam(TEX_0, HeightMap); // 지형 높이맵
//
...
Vec3 FLandscape::GetWorldPosByLandScape(const Vec3& InTargetWorldPos) const
{
	// 랜드스케이프의 월드 행렬
	const Matrix& matWorld = Transform()->GetWorldMat();
	...
	Vec3 localPos = XMVector3TransformCoord(InTargetWorldPos, matWorldInv);

	// 로컬 좌표를 랜드스케이프 텍스쳐 UV 좌표로 변환
	// 랜드스케이프의 X, Z 크기에 따라 조정
	float u = localPos.x / static_cast<float>(FaceX);
	float v = 1.f - (localPos.z / static_cast<float>(FaceZ)); // v 좌표는 상하가 반전되어 있으므로 1에서 빼줌

	// 로컬 좌표가 랜드스케이프 범위 내에 있는지 확인
	if (localPos.x < 0.f || localPos.x >static_cast<float>(FaceX) ||
		localPos.z < 0.f || localPos.z >static_cast<float>(FaceZ))
	{
		// 범위를 벗어나면 큰 음수 값을 반환
	...
	{
		// 높이맵 텍스처에서 해당 UV 위치의 높이 값을 가져옴
		UINT heightMapWidth = HeightMap->GetWidth();
		UINT heightMapHeight = HeightMap->GetHeight();
		...
		// 높이값 추출
		// 텍스처에서 높이 값 가져오기
		height = CachedHeightData[y * heightMapWidth + x];
	}
...
Vec3 FLandscape::GetWorldPosLandNormal(Vec3& InTargetWorldPos) const
{
	// 랜드스케이프의 월드 행렬
	const Matrix& matWorld = Transform()->GetWorldMat();
	...
	// 높이값 추출
	// 텍스처에서 높이 값 가져오기
	float heightCenter = CachedHeightData[y * heightMapWidth + x];
	float heightRight = CachedHeightData[y * heightMapWidth + xRight];
	float heightLeft = CachedHeightData[y * heightMapWidth + xLeft];
	float heightUp = CachedHeightData[yUp * heightMapWidth + x];
	float heightDown = CachedHeightData[yDown * heightMapWidth + x];

	// x벡터, z벡터 생성
	Vec3 vecX = Vec3(static_cast<float>(xRight - xLeft) * static_cast<float>(FaceX) / static_cast<float>(heightMapWidth - 1),
					heightRight - heightLeft,
					0.0f);

	Vec3 vecZ = Vec3(0.0f,
					heightUp - heightDown,
					static_cast<float>(yUp - yDown) * static_cast<float>(FaceZ) / static_cast<float>(heightMapHeight - 1));
	...
	InTargetWorldPos = XMVector3TransformCoord(localPos, matWorld);
...
{
	(void)fwrite(&FaceX, sizeof(UINT), 1, InFile);
	(void)fwrite(&FaceZ, sizeof(UINT), 1, InFile);
	SaveAssetRef(HeightMap, InFile);

	// TODO(KHJ): 다 정리되면 Dummy 제거
	Ptr<CTexture> Dummy1;
	Ptr<CTexture> Dummy2;
	SaveAssetRef(Dummy1, InFile);
	SaveAssetRef(Dummy2, InFile);
}

void FLandscape::LoadComponent(FILE* InFile)
{
	(void)fread(&FaceX, sizeof(UINT), 1, InFile);
	(void)fread(&FaceZ, sizeof(UINT), 1, InFile);
	CreateMesh();
	LoadAssetRef(HeightMap, InFile);
	HeightMap->CaptureTextureCustom(CachedHeightData);

	// TODO(KHJ): 다 정리되면 Dummy 제거
	Ptr<CTexture> Dummy1;
	Ptr<CTexture> Dummy2;
	LoadAssetRef(Dummy1, InFile);
	LoadAssetRef(Dummy2, InFile);
}
