...
static void ImGui_ImplWin32_UpdateMouseData()
{
    ...
    const bool is_app_focused = (focused_window && (focused_window == bd->hWnd ||
        IsChild(focused_window, bd->hWnd) || ImGui::FindViewportByPlatformHandle(focused_window)));
    if (is_app_focused)
    {
...
static void ImGui_ImplWin32_UpdateGamepads()
{
    ...
    if (!bd->HasGamepad || bd->XInputGetState == nullptr || bd->XInputGetState(0, &xinput_state) !=
        ERROR_SUCCESS)
        return;
    io.BackendFlags |= ImGuiBackendFlags_HasGamepad;
...
static BOOL CALLBACK ImGui_ImplWin32_UpdateMonitors_EnumFunc(HMONITOR monitor, HDC, LPRECT, LPARAM)
{
    ...
    imgui_monitor.MainPos = ImVec2(static_cast<float>(info.rcMonitor.left),
                                   static_cast<float>(info.rcMonitor.top));
    imgui_monitor.MainSize = ImVec2(static_cast<float>(info.rcMonitor.right - info.rcMonitor.left),
                                    static_cast<float>(info.rcMonitor.bottom - info.rcMonitor.top));
    imgui_monitor.WorkPos = ImVec2(static_cast<float>(info.rcWork.left),
                                   static_cast<float>(info.rcWork.top));
    imgui_monitor.WorkSize = ImVec2(static_cast<float>(info.rcWork.right - info.rcWork.left),
                                    static_cast<float>(info.rcWork.bottom - info.rcWork.top));
    imgui_monitor.DpiScale = ImGui_ImplWin32_GetDpiScaleForMonitor(monitor);
    imgui_monitor.PlatformHandle = static_cast<void*>(monitor);
                                                      ...
                                                      LPARAM lParam)
{
    ...
    case WM_NCMOUSEMOVE:
        {
            ...
            if (bd->MouseTrackedArea != area)
            {
                ...
                TRACKMOUSEEVENT tme_track = {
                    sizeof(tme_track),
                    static_cast<DWORD>((area == 2) ? (TME_LEAVE | TME_NONCLIENT) : TME_LEAVE), hwnd,
                    0
                };
            ...
            POINT mouse_pos = {
                static_cast<LONG>(GET_X_LPARAM(lParam)), static_cast<LONG>(GET_Y_LPARAM(lParam))
            };
            bool want_absolute_pos = (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable) != 0;
            if (msg == WM_MOUSEMOVE && want_absolute_pos)
                // WM_MOUSEMOVE are client-relative coordinates.
                ClientToScreen(hwnd, &mouse_pos);
            if (msg == WM_NCMOUSEMOVE && !want_absolute_pos)
                // WM_NCMOUSEMOVE are absolute coordinates.
    ...
    case WM_MOUSEWHEEL:
        io.AddMouseWheelEvent(
            0.0f, static_cast<float>(GET_WHEEL_DELTA_WPARAM(wParam)) / static_cast<float>(
                WHEEL_DELTA));
        return 0;
    case WM_MOUSEHWHEEL:
        io.AddMouseWheelEvent(
...
static BOOL _IsWindowsVersionOrGreater(WORD major, WORD minor, WORD)
{
    ...
    return (RtlVerifyVersionInfoFn(&versionInfo, VER_MAJORVERSION | VER_MINORVERSION, conditionMask)
               == 0)
               ? TRUE
               : FALSE;
}
...
void ImGui_ImplWin32_EnableDpiAwareness()
{
    ...
    if (_IsWindows10OrGreater())
    {
        ...
        if (auto SetThreadDpiAwarenessContextFn = (PFN_SetThreadDpiAwarenessContext)GetProcAddress(
    ...
    if (_IsWindows8Point1OrGreater())
    {
        ...
        if (auto SetProcessDpiAwarenessFn = (PFN_SetProcessDpiAwareness)GetProcAddress(
...
static HWND ImGui_ImplWin32_GetHwndFromViewportID(ImGuiID viewport_id)
{
    if (viewport_id != 0)
        if (ImGuiViewport* viewport = ImGui::FindViewportByID(viewport_id))
            return static_cast<HWND>(viewport->PlatformHandle);
    return nullptr;
}

static void ImGui_ImplWin32_CreateWindow(ImGuiViewport* viewport)
{
    auto vd = IM_NEW(ImGui_ImplWin32_ViewportData)();
    ...
    // Create window
    RECT rect = {
        static_cast<LONG>(viewport->Pos.x), static_cast<LONG>(viewport->Pos.y),
        static_cast<LONG>(viewport->Pos.x + viewport->Size.x),
        static_cast<LONG>(viewport->Pos.y + viewport->Size.y)
    };
    AdjustWindowRectEx(&rect, vd->DwStyle, FALSE, vd->DwExStyle);
    vd->Hwnd = ::CreateWindowEx(
        vd->DwExStyle, _T("ImGui Platform"), _T("Untitled"), vd->DwStyle,
        // Style, class name, window name
...
{
    ImGui_ImplWin32_Data* bd = ImGui_ImplWin32_GetBackendData();
    if (auto vd = static_cast<ImGui_ImplWin32_ViewportData*>(viewport->PlatformUserData))
    {
...
{
    auto vd = static_cast<ImGui_ImplWin32_ViewportData*>(viewport->PlatformUserData);
    IM_ASSERT(vd->Hwnd != 0);
...
static void ImGui_ImplWin32_UpdateWindow(ImGuiViewport* viewport)
{
    ...
    if (vd->DwStyle != new_style || vd->DwExStyle != new_ex_style)
    {
        ...
        HWND insert_after = top_most_changed
        ...
        SetWindowPos(vd->Hwnd, insert_after, rect.left, rect.top, rect.right - rect.left,
                     rect.bottom - rect.top, swp_flag | SWP_NOACTIVATE | SWP_FRAMECHANGED);
...
static void ImGui_ImplWin32_SetWindowPos(ImGuiViewport* viewport, ImVec2 pos)
{
    ...
    IM_ASSERT(vd->Hwnd != 0);
    RECT rect = {
        static_cast<LONG>(pos.x), static_cast<LONG>(pos.y), static_cast<LONG>(pos.x),
...
static ImVec2 ImGui_ImplWin32_GetWindowSize(ImGuiViewport* viewport)
{
    ...
    return ImVec2(static_cast<float>(rect.right - rect.left),
                  static_cast<float>(rect.bottom - rect.top));
}
...
static void ImGui_ImplWin32_SetWindowAlpha(ImGuiViewport* viewport, float alpha)
{
    ...
    if (alpha < 1.0f)
    {
        ...
        SetLayeredWindowAttributes(vd->Hwnd, 0, static_cast<BYTE>(255 * alpha), LWA_ALPHA);
    }
    else
    {
        DWORD style = GetWindowLongW(vd->Hwnd, GWL_EXSTYLE) & ~WS_EX_LAYERED;
...
static void ImGui_ImplWin32_InitPlatformInterface(bool platform_has_own_dc)
{
    ...
    main_viewport->PlatformHandle = static_cast<void*>(bd->hWnd);
}
