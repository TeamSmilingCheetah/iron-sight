...
// Widgets
static constexpr float DRAGDROP_HOLD_TO_OPEN_TIMER = 0.70f;
// Time for drag-hold to activate items accepting the ImGuiButtonFlags_PressedOnDragDropHold button behavior.
static constexpr float DRAG_MOUSE_THRESHOLD_FACTOR = 0.50f;
...
static ImVec2 InputTextCalcTextSizeW(ImGuiContext* ctx, const ImWchar* text_begin,
                                     const ImWchar* text_end, const ImWchar** remaining = nullptr,
                                     ImVec2* out_offset = nullptr, bool stop_on_new_line = false);
...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    else
    {
        ...
        if (!g.LogEnabled)
        {
            int lines_skippable = static_cast<int>((window->ClipRect.Min.y - text_pos.y) /
                line_height);
            if (lines_skippable > 0)
            {
                ...
                {
                    auto line_end = static_cast<const char*>(memchr(line, '\n', text_end - line));
                    if (!line_end)
                        line_end = text_end;
                           ...
                           ImGuiButtonFlags flags)
{
    ...
    ImGuiItemFlags item_flags = (g.LastItemData.ID == id
    ...
    const bool flatten_hovered_children = (flags & ImGuiButtonFlags_FlattenChildren) && g.
        HoveredWindow && g.HoveredWindow->RootWindowDockTree == window->RootWindowDockTree;
    ...
    if (g.DragDropActive && (flags & ImGuiButtonFlags_PressedOnDragDropHold) && !(g.
        DragDropSourceFlags & ImGuiDragDropFlags_SourceNoHoldToOpenOthers))
    ...
    if (hovered)
    {
        ...
        if (!(flags & ImGuiButtonFlags_NoKeyModifiers) || (!g.IO.KeyCtrl && !g.IO.KeyShift && !g.IO.
            KeyAlt))
        {
            if (mouse_button_clicked != -1 && g.ActiveId != id)
            {
                ...
                if ((flags & ImGuiButtonFlags_PressedOnClick) || ((flags &
                    ImGuiButtonFlags_PressedOnDoubleClick) && g.IO.MouseClickedCount[
            ...
            if (flags & ImGuiButtonFlags_PressedOnRelease)
            {
                ...
                {
                    const bool has_repeated_at_least_once = (item_flags &
                        ImGuiItemFlags_ButtonRepeat) && g.IO.MouseDownDurationPrev[
    ...
    if (g.NavId == id && !g.NavDisableHighlight && g.NavDisableMouseHover && (g.ActiveId == 0 || g.
        ActiveId == id || g.ActiveId == window->MoveId))
    ...
    if (g.NavActivateDownId == id)
    {
        ...
        if (!nav_activated_by_inputs && (item_flags & ImGuiItemFlags_ButtonRepeat))
        {
            ...
            nav_activated_by_inputs = CalcTypematicRepeatAmount(
                t1 - g.IO.DeltaTime, t1, g.IO.KeyRepeatDelay, g.IO.KeyRepeatRate) > 0;
        }
    ...
    if (g.ActiveId == id)
    {
            ...
            else
            {
                ...
                if ((release_in || release_anywhere) && !g.DragDropActive)
                {
                    ...
                    bool is_double_click_release = (flags & ImGuiButtonFlags_PressedOnDoubleClick)
                        && g.IO.MouseReleased[mouse_button] && g.IO.MouseClickedLastCount[
                            mouse_button] == 2;
                    bool is_repeating_already = (item_flags & ImGuiItemFlags_ButtonRepeat) && g.IO.
                        MouseDownDurationPrev[mouse_button] >= g.IO.KeyRepeatDelay;
        ...
        else if (g.ActiveIdSource == ImGuiInputSource_Keyboard || g.ActiveIdSource ==
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, nullptr,
...
bool ImGui::ArrowButtonEx(const char* str_id, ImGuiDir dir, ImVec2 size, ImGuiButtonFlags flags)
{
    ...
    RenderArrow(window->DrawList,
                bb.Min + ImVec2(ImMax(0.0f, (size.x - g.FontSize) * 0.5f),
                                ImMax(0.0f, (size.y - g.FontSize) * 0.5f)), text_col, dir);
...
void ImGui::Scrollbar(ImGuiAxis axis)
{
    ...
    {
        if ((window->Flags & ImGuiWindowFlags_NoTitleBar) && !(window->Flags &
    ...
    ImS64 scroll = window->Scroll[axis];
    ScrollbarEx(bb, id, axis, &scroll, size_avail, size_contents, rounding_corners);
                        ...
                        ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    const float grab_h_pixels = ImClamp(
        scrollbar_size_v * (static_cast<float>(size_avail_v) / static_cast<float>(win_size_v)),
    ...
    float scroll_ratio = ImSaturate(
        static_cast<float>(*p_scroll_v) / static_cast<float>(scroll_max));
    ...
    if (held && allow_interaction && grab_h_norm < 1.0f)
    {
        ...
        if (g.ActiveIdIsJustActivated)
        {
            ...
            seek_absolute = (clicked_v_norm < grab_v_norm || clicked_v_norm > grab_v_norm +
                grab_h_norm);
            if (seek_absolute)
                g.ScrollbarClickDeltaToGrabCenter = 0.0f;
            else
                g.ScrollbarClickDeltaToGrabCenter = clicked_v_norm - grab_v_norm - grab_h_norm *
                    0.5f;
        }
        ...
        const float scroll_v_norm = ImSaturate(
            (clicked_v_norm - g.ScrollbarClickDeltaToGrabCenter - grab_h_norm * 0.5f) / (1.0f -
                grab_h_norm));
                          ...
                          const ImVec4& tint_col, ImGuiButtonFlags flags)
{
    ...
    RenderFrame(bb.Min, bb.Max, col, true,
                ImClamp(ImMin(padding.x, padding.y), 0.0f, g.Style.FrameRounding));
    if (bg_col.w > 0.0f)
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    auto label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x,
    ...
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label,
                                g.LastItemData.StatusFlags | ImGuiItemStatusFlags_Checkable | (*v ?
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    if (overlay_size.x > 0.0f)
        RenderTextClipped(
            ...
            overlay, nullptr, &overlay_size, ImVec2(0.0f, 0.5f), &bb);
}
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness)
{
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        // FIXME: See 1.70/1.71 Separator() change: makes legacy 1-px separator not affect layout yet. Should change.
...
void ImGui::SeparatorTextEx(ImGuiID id, const char* label, const char* label_end, float extra_w)
{
    ...
    const float text_baseline_y = ImTrunc(
        (bb.GetHeight() - label_size.y) * style.SeparatorTextAlign.y + 0.99999f);
    //ImMax(padding.y, ImTrunc((style.SeparatorTextSize - label_size.y) * 0.5f));
    ...
    const ImVec2 label_pos(
        pos.x + padding.x + ImMax(
            0.0f, (label_avail_w - label_size.x - extra_w) * style.SeparatorTextAlign.x),
                             ...
                             float hover_visibility_delay, ImU32 bg_col)
{
    ...
    if (held || (hovered && g.HoveredIdPreviousFrame == id && g.HoveredIdTimer >=
...
{
    auto a = static_cast<const ImGuiShrinkWidthItem*>(lhs);
    auto b = static_cast<const ImGuiShrinkWidthItem*>(rhs);
...
void ImGui::ShrinkWidths(ImGuiShrinkWidthItem* items, int count, float width_excess)
{
    ...
    ImQsort(items, static_cast<size_t>(count), sizeof(ImGuiShrinkWidthItem),
    ...
    while (width_excess > 0.0f && count_same_width < count)
    {
        while (count_same_width < count && items[0].Width <= items[count_same_width].Width)
            count_same_width++;
        float max_width_to_remove_per_item = (count_same_width < count && items[count_same_width].
                                                 Width >= 0.0f)
...
static float CalcMaxPopupHeightFromItemCount(int items_count)
{
    ...
    if (items_count <= 0)
        return FLT_MAX;
    return (g.FontSize + g.Style.ItemSpacing.y) * items_count - g.Style.ItemSpacing.y + (g.Style.
        WindowPadding.y * 2);
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (flags & ImGuiComboFlags_WidthFitPreview)
        IM_ASSERT(
        (flags & (ImGuiComboFlags_NoPreview | static_cast<ImGuiComboFlags>(
            ImGuiComboFlags_CustomPreview))) == 0);

    const float arrow_size = (flags & ImGuiComboFlags_NoArrowButton) ? 0.0f : GetFrameHeight();
    const ImVec2 label_size = CalcTextSize(label, nullptr, true);
    const float preview_width = ((flags & ImGuiComboFlags_WidthFitPreview) && (preview_value !=
                                    nullptr))
                                    ? CalcTextSize(preview_value, nullptr, true).x
                                    : 0.0f;
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    {
        g.NextWindowData.SizeConstraintRect.Min.x = ImMax(g.NextWindowData.SizeConstraintRect.Min.x,
                                                          w);
    }
    else
    {
        ...
        if ((g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSize) == 0 || g.NextWindowData.
            SizeVal.x <= 0.0f) // Don't apply constraints if user specified a size
            constraint_min.x = w;
        if ((g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSize) == 0 || g.NextWindowData.
            SizeVal.y <= 0.0f)
    ...
    PushStyleVar(ImGuiStyleVar_WindowPadding,
                 ImVec2(g.Style.FramePadding.x, g.Style.WindowPadding.y));
...
bool ImGui::BeginComboPreview()
{
    ...
    if (window->SkipItems || !(g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible))
        return false;
    IM_ASSERT(
        g.LastItemData.Rect.Min.x == preview_data->PreviewRect.Min.x && g.LastItemData.Rect.Min.y ==
        preview_data->PreviewRect.Min.y);
...
void ImGui::EndComboPreview()
{
    ...
    if (window->DC.CursorMaxPos.x < preview_data->PreviewRect.Max.x && window->DC.CursorMaxPos.y <
        preview_data->PreviewRect.Max.y)
        if (draw_list->CmdBuffer.Size > 1)
        // Unlikely case that the PushClipRect() didn't create a command
        {
            draw_list->_CmdHeader.ClipRect = draw_list->CmdBuffer[draw_list->CmdBuffer.Size - 1].
                ClipRect = draw_list->CmdBuffer[draw_list->CmdBuffer.Size - 2].ClipRect;
...
{
    auto items = static_cast<const char* const*>(data);
    return items[idx];
}
...
{
    auto items_separated_by_zeros = static_cast<const char*>(data);
    int items_count = 0;
                  ...
                  int popup_max_height_in_items)
{
    ...
    if (popup_max_height_in_items != -1 && !(g.NextWindowData.Flags &
...
{
    auto data = static_cast<ImGuiGetNameFromIndexOldToNewCallbackData*>(user_data);
                            ...
                            const void* arg2)
{
    ...
    case ImGuiDataType_S8:
        if (op == '+')
        {
            *static_cast<ImS8*>(output) = ImAddClampOverflow(
                *static_cast<const ImS8*>(arg1), *static_cast<const ImS8*>(arg2), IM_S8_MIN,
                IM_S8_MAX);
        }
        if (op == '-')
        {
            *static_cast<ImS8*>(output) = ImSubClampOverflow(
                *static_cast<const ImS8*>(arg1), *static_cast<const ImS8*>(arg2), IM_S8_MIN,
                IM_S8_MAX);
        }
        return;
    case ImGuiDataType_U8:
        if (op == '+')
        {
            *static_cast<ImU8*>(output) = ImAddClampOverflow(
                *static_cast<const ImU8*>(arg1), *static_cast<const ImU8*>(arg2), IM_U8_MIN,
                IM_U8_MAX);
        }
        if (op == '-')
        {
            *static_cast<ImU8*>(output) = ImSubClampOverflow(
                *static_cast<const ImU8*>(arg1), *static_cast<const ImU8*>(arg2), IM_U8_MIN,
                IM_U8_MAX);
        }
        return;
    case ImGuiDataType_S16:
        if (op == '+')
        {
            *static_cast<ImS16*>(output) = ImAddClampOverflow(
                *static_cast<const ImS16*>(arg1), *static_cast<const ImS16*>(arg2), IM_S16_MIN,
                IM_S16_MAX);
        }
        if (op == '-')
        {
            *static_cast<ImS16*>(output) = ImSubClampOverflow(
                *static_cast<const ImS16*>(arg1), *static_cast<const ImS16*>(arg2), IM_S16_MIN,
                IM_S16_MAX);
        }
        return;
    case ImGuiDataType_U16:
        if (op == '+')
        {
            *static_cast<ImU16*>(output) = ImAddClampOverflow(
                *static_cast<const ImU16*>(arg1), *static_cast<const ImU16*>(arg2), IM_U16_MIN,
                IM_U16_MAX);
        }
        if (op == '-')
        {
            *static_cast<ImU16*>(output) = ImSubClampOverflow(
                *static_cast<const ImU16*>(arg1), *static_cast<const ImU16*>(arg2), IM_U16_MIN,
                IM_U16_MAX);
        }
        return;
    case ImGuiDataType_S32:
        if (op == '+')
        {
            *static_cast<ImS32*>(output) = ImAddClampOverflow(
                *static_cast<const ImS32*>(arg1), *static_cast<const ImS32*>(arg2), IM_S32_MIN,
                IM_S32_MAX);
        }
        if (op == '-')
        {
            *static_cast<ImS32*>(output) = ImSubClampOverflow(
                *static_cast<const ImS32*>(arg1), *static_cast<const ImS32*>(arg2), IM_S32_MIN,
                IM_S32_MAX);
        }
        return;
    case ImGuiDataType_U32:
        if (op == '+')
        {
            *static_cast<ImU32*>(output) = ImAddClampOverflow(
                *static_cast<const ImU32*>(arg1), *static_cast<const ImU32*>(arg2), IM_U32_MIN,
                IM_U32_MAX);
        }
        if (op == '-')
        {
            *static_cast<ImU32*>(output) = ImSubClampOverflow(
                *static_cast<const ImU32*>(arg1), *static_cast<const ImU32*>(arg2), IM_U32_MIN,
                IM_U32_MAX);
        }
        return;
    case ImGuiDataType_S64:
        if (op == '+')
        {
            *static_cast<ImS64*>(output) = ImAddClampOverflow(
                *static_cast<const ImS64*>(arg1), *static_cast<const ImS64*>(arg2), IM_S64_MIN,
                IM_S64_MAX);
        }
        if (op == '-')
        {
            *static_cast<ImS64*>(output) = ImSubClampOverflow(
                *static_cast<const ImS64*>(arg1), *static_cast<const ImS64*>(arg2), IM_S64_MIN,
                IM_S64_MAX);
        }
        return;
    case ImGuiDataType_U64:
        if (op == '+')
        {
            *static_cast<ImU64*>(output) = ImAddClampOverflow(
                *static_cast<const ImU64*>(arg1), *static_cast<const ImU64*>(arg2), IM_U64_MIN,
                IM_U64_MAX);
        }
        if (op == '-')
        {
            *static_cast<ImU64*>(output) = ImSubClampOverflow(
                *static_cast<const ImU64*>(arg1), *static_cast<const ImU64*>(arg2), IM_U64_MIN,
                IM_U64_MAX);
        }
        return;
    case ImGuiDataType_Float:
        if (op == '+')
        {
            *static_cast<float*>(output) = *static_cast<const float*>(arg1) + *static_cast<const
                float*>(arg2);
        }
        if (op == '-')
        {
    ...
    case ImGuiDataType_Double:
        if (op == '+')
        {
            *static_cast<double*>(output) = *static_cast<const double*>(arg1) + *static_cast<const
                double*>(arg2);
        }
        if (op == '-')
        {
                                  ...
                                  const char* format)
{
    ...
    {
        if (data_type == ImGuiDataType_S8)
            *static_cast<ImS8*>(p_data) = static_cast<ImS8>(ImClamp(
                v32, (int)IM_S8_MIN, (int)IM_S8_MAX));
        else if (data_type == ImGuiDataType_U8)
            *static_cast<ImU8*>(p_data) = static_cast<ImU8>(ImClamp(
                v32, (int)IM_U8_MIN, (int)IM_U8_MAX));
        else if (data_type == ImGuiDataType_S16)
            *static_cast<ImS16*>(p_data) = static_cast<ImS16>(ImClamp(
                v32, (int)IM_S16_MIN, (int)IM_S16_MAX));
        else if (data_type == ImGuiDataType_U16)
            *static_cast<ImU16*>(p_data) = static_cast<ImU16>(ImClamp(
                v32, (int)IM_U16_MIN, (int)IM_U16_MAX));
        else
            IM_ASSERT(0);
    }
...
int ImGui::DataTypeCompare(ImGuiDataType data_type, const void* arg_1, const void* arg_2)
{
    switch (data_type)
    {
    case ImGuiDataType_S8: return DataTypeCompareT<ImS8>(static_cast<const ImS8*>(arg_1),
                                                         static_cast<const ImS8*>(arg_2));
    case ImGuiDataType_U8: return DataTypeCompareT<ImU8>(static_cast<const ImU8*>(arg_1),
                                                         static_cast<const ImU8*>(arg_2));
    case ImGuiDataType_S16: return DataTypeCompareT<ImS16>(static_cast<const ImS16*>(arg_1),
                                                           static_cast<const ImS16*>(arg_2));
    case ImGuiDataType_U16: return DataTypeCompareT<ImU16>(static_cast<const ImU16*>(arg_1),
                                                           static_cast<const ImU16*>(arg_2));
    case ImGuiDataType_S32: return DataTypeCompareT<ImS32>(static_cast<const ImS32*>(arg_1),
                                                           static_cast<const ImS32*>(arg_2));
    case ImGuiDataType_U32: return DataTypeCompareT<ImU32>(static_cast<const ImU32*>(arg_1),
                                                           static_cast<const ImU32*>(arg_2));
    case ImGuiDataType_S64: return DataTypeCompareT<ImS64>(static_cast<const ImS64*>(arg_1),
                                                           static_cast<const ImS64*>(arg_2));
    case ImGuiDataType_U64: return DataTypeCompareT<ImU64>(static_cast<const ImU64*>(arg_1),
                                                           static_cast<const ImU64*>(arg_2));
    case ImGuiDataType_Float: return DataTypeCompareT<float>(
            static_cast<const float*>(arg_1), static_cast<const float*>(arg_2));
    case ImGuiDataType_Double: return DataTypeCompareT<double>(
            static_cast<const double*>(arg_1), static_cast<const double*>(arg_2));
    case ImGuiDataType_COUNT: break;
    }
    IM_ASSERT(0);
    return 0;
                          ...
                          const void* p_max)
{
    switch (data_type)
    {
    case ImGuiDataType_S8: return DataTypeClampT<ImS8>(static_cast<ImS8*>(p_data),
                                                       static_cast<const ImS8*>(p_min),
                                                       static_cast<const ImS8*>(p_max));
    case ImGuiDataType_U8: return DataTypeClampT<ImU8>(static_cast<ImU8*>(p_data),
                                                       static_cast<const ImU8*>(p_min),
                                                       static_cast<const ImU8*>(p_max));
    case ImGuiDataType_S16: return DataTypeClampT<ImS16>(static_cast<ImS16*>(p_data),
                                                         static_cast<const ImS16*>(p_min),
                                                         static_cast<const ImS16*>(p_max));
    case ImGuiDataType_U16: return DataTypeClampT<ImU16>(static_cast<ImU16*>(p_data),
                                                         static_cast<const ImU16*>(p_min),
                                                         static_cast<const ImU16*>(p_max));
    case ImGuiDataType_S32: return DataTypeClampT<ImS32>(static_cast<ImS32*>(p_data),
                                                         static_cast<const ImS32*>(p_min),
                                                         static_cast<const ImS32*>(p_max));
    case ImGuiDataType_U32: return DataTypeClampT<ImU32>(static_cast<ImU32*>(p_data),
                                                         static_cast<const ImU32*>(p_min),
                                                         static_cast<const ImU32*>(p_max));
    case ImGuiDataType_S64: return DataTypeClampT<ImS64>(static_cast<ImS64*>(p_data),
                                                         static_cast<const ImS64*>(p_min),
                                                         static_cast<const ImS64*>(p_max));
    case ImGuiDataType_U64: return DataTypeClampT<ImU64>(static_cast<ImU64*>(p_data),
                                                         static_cast<const ImU64*>(p_min),
                                                         static_cast<const ImU64*>(p_max));
    case ImGuiDataType_Float: return DataTypeClampT<float>(static_cast<float*>(p_data),
                                                           static_cast<const float*>(p_min),
                                                           static_cast<const float*>(p_max));
    case ImGuiDataType_Double: return DataTypeClampT<double>(
            static_cast<double*>(p_data), static_cast<const double*>(p_min),
            static_cast<const double*>(p_max));
    case ImGuiDataType_COUNT: break;
    }
    IM_ASSERT(0);
    return false;
...
{
    static constexpr float min_steps[10] = {
        1.0f, 0.1f, 0.01f, 0.001f, 0.0001f, 0.00001f, 0.000001f, 0.0000001f, 0.00000001f,
        0.000000001f
    };
    ...
    return (decimal_precision < IM_ARRAYSIZE(min_steps))
               ...
               : ImPow(10.0f, static_cast<float>(-decimal_precision));
}
                          ...
                          const TYPE v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const bool is_floating_point = (data_type == ImGuiDataType_Float) || (data_type ==
    ...
    bool is_already_past_limits_and_pushing_outward = is_clamped && ((*v >= v_max && adjust_delta >
        0.0f) || (*v <= v_min && adjust_delta < 0.0f));
    ...
    constexpr float zero_deadzone_halfsize = 0.0f;
    // Drag widgets have no deadzone (as it doesn't make sense)
    if (is_logarithmic)
    {
                         ...
                         ImGuiSliderFlags flags)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        if (g.ActiveIdSource == ImGuiInputSource_Mouse && !g.IO.MouseDown[0])
            ...
            ImGuiInputSource_Gamepad) && g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated)
            ClearActiveID();
    }
    if (g.ActiveId != id)
        return false;
    ...
    case ImGuiDataType_S8:
        {
            ImS32 v32 = *static_cast<ImS8*>(p_v);
            bool r = DragBehaviorT<ImS32, ImS32, float>(ImGuiDataType_S32, &v32, v_speed,
                                                        ...
                                                        p_max
                                                            ? *static_cast<const ImS8*>(p_max)
                                                            : IM_S8_MAX, format, flags);
            if (r) *static_cast<ImS8*>(p_v) = static_cast<ImS8>(v32);
            return r;
        }
    case ImGuiDataType_U8:
        {
            ImU32 v32 = *static_cast<ImU8*>(p_v);
            bool r = DragBehaviorT<ImU32, ImS32, float>(ImGuiDataType_U32, &v32, v_speed,
                                                        ...
                                                        p_max
                                                            ? *static_cast<const ImU8*>(p_max)
                                                            : IM_U8_MAX, format, flags);
            if (r) *static_cast<ImU8*>(p_v) = static_cast<ImU8>(v32);
            return r;
        }
    case ImGuiDataType_S16:
        {
            ImS32 v32 = *static_cast<ImS16*>(p_v);
            bool r = DragBehaviorT<ImS32, ImS32, float>(ImGuiDataType_S32, &v32, v_speed,
                                                        p_min
                                                            ? *static_cast<const ImS16*>(p_min)
                                                            : IM_S16_MIN,
                                                        p_max
                                                            ? *static_cast<const ImS16*>(p_max)
                                                            : IM_S16_MAX, format, flags);
            if (r) *static_cast<ImS16*>(p_v) = static_cast<ImS16>(v32);
            return r;
        }
    case ImGuiDataType_U16:
        {
            ImU32 v32 = *static_cast<ImU16*>(p_v);
            bool r = DragBehaviorT<ImU32, ImS32, float>(ImGuiDataType_U32, &v32, v_speed,
                                                        p_min
                                                            ? *static_cast<const ImU16*>(p_min)
                                                            : IM_U16_MIN,
                                                        p_max
                                                            ? *static_cast<const ImU16*>(p_max)
                                                            : IM_U16_MAX, format, flags);
            if (r) *static_cast<ImU16*>(p_v) = static_cast<ImU16>(v32);
            return r;
        }
    case ImGuiDataType_S32: return DragBehaviorT<ImS32, ImS32, float>(
            data_type, static_cast<ImS32*>(p_v), v_speed,
            p_min ? *static_cast<const ImS32*>(p_min) : IM_S32_MIN,
            p_max ? *static_cast<const ImS32*>(p_max) : IM_S32_MAX, format, flags);
    case ImGuiDataType_U32: return DragBehaviorT<ImU32, ImS32, float>(
            data_type, static_cast<ImU32*>(p_v), v_speed,
            p_min ? *static_cast<const ImU32*>(p_min) : IM_U32_MIN,
            p_max ? *static_cast<const ImU32*>(p_max) : IM_U32_MAX, format, flags);
    case ImGuiDataType_S64: return DragBehaviorT<ImS64, ImS64, double>(
            data_type, static_cast<ImS64*>(p_v), v_speed,
            p_min ? *static_cast<const ImS64*>(p_min) : IM_S64_MIN,
            p_max ? *static_cast<const ImS64*>(p_max) : IM_S64_MAX, format, flags);
    case ImGuiDataType_U64: return DragBehaviorT<ImU64, ImS64, double>(
            data_type, static_cast<ImU64*>(p_v), v_speed,
            p_min ? *static_cast<const ImU64*>(p_min) : IM_U64_MIN,
            p_max ? *static_cast<const ImU64*>(p_max) : IM_U64_MAX, format, flags);
    case ImGuiDataType_Float: return DragBehaviorT<float, float, float>(
            data_type, static_cast<float*>(p_v), v_speed,
            p_min ? *static_cast<const float*>(p_min) : -FLT_MAX,
            p_max ? *static_cast<const float*>(p_max) : FLT_MAX, format, flags);
    case ImGuiDataType_Double: return DragBehaviorT<double, double, double>(
            data_type, static_cast<double*>(p_v), v_speed,
            p_min ? *static_cast<const double*>(p_min) : -DBL_MAX,
            p_max ? *static_cast<const double*>(p_max) : DBL_MAX, format, flags);
    case ImGuiDataType_COUNT: break;
    }
    IM_ASSERT(0);
    return false;
                       ...
                       ImGuiSliderFlags flags)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos,
                          window->DC.CursorPos + ImVec2(
                              w, label_size.y + style.FramePadding.y * 2.0f));
    ...
    if (!temp_input_is_active)
    {
        ...
        const bool double_clicked = (hovered && g.IO.MouseClickedCount[0] == 2 && TestKeyOwner(
            ImGuiKey_MouseLeft, id));
        const bool make_active = (clicked || double_clicked || g.NavActivateId == id);
        if (make_active && (clicked || double_clicked))
            SetKeyOwner(ImGuiKey_MouseLeft, id);
        if (make_active && temp_input_allowed)
            if ((clicked && g.IO.KeyCtrl) || double_clicked || (g.NavActivateId == id && (g.
                NavActivateFlags & ImGuiActivateFlags_PreferInput)))
    ...
    if (temp_input_is_active)
    {
        ...
        const bool is_clamp_input = (flags & ImGuiSliderFlags_AlwaysClamp) != 0 && (p_min == nullptr
            || p_max == nullptr || DataTypeCompare(data_type, p_min, p_max) < 0);
        return TempInputScalar(frame_bb, id, label, data_type, p_data, format,
                               is_clamp_input ? p_min : nullptr, is_clamp_input ? p_max : nullptr);
    }
    ...
    // Drag behavior
    const bool value_changed = DragBehavior(id, data_type, p_data, v_speed, p_min, p_max, format,
                                            flags);
    if (value_changed)
        MarkItemEdited(id);
    ...
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(
        value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    if (g.LogEnabled)
        LogSetNextTextDecoration("{", "}");
    RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, nullptr,
                      ImVec2(0.5f, 0.5f));
    ...
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label,
                                g.LastItemData.StatusFlags | (temp_input_allowed ?
                                    ImGuiItemStatusFlags_Inputable : 0));
    return value_changed;
}
                          ...
                          ImGuiSliderFlags flags)
{
    ...
    ImGuiSliderFlags min_flags = flags | ((min_min == min_max) ? ImGuiSliderFlags_ReadOnly : 0);
    bool value_changed = DragInt("##min", v_current_min, v_speed, min_min, min_max, format,
                                 min_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
                                  ...
                                  float zero_deadzone_halfsize)
{
    ...
    if (is_logarithmic)
    {
        ...
        FLOATTYPE v_min_fudged = (ImAbs(static_cast<FLOATTYPE>(v_min)) < logarithmic_zero_epsilon)
                                     ? ((v_min < 0.0f)
                                            ? -logarithmic_zero_epsilon
                                            : logarithmic_zero_epsilon)
                                     : static_cast<FLOATTYPE>(v_min);
        FLOATTYPE v_max_fudged = (ImAbs(static_cast<FLOATTYPE>(v_max)) < logarithmic_zero_epsilon)
                                     ? ((v_max < 0.0f)
                                            ? -logarithmic_zero_epsilon
                                            : logarithmic_zero_epsilon)
                                     : static_cast<FLOATTYPE>(v_max);
        ...
        {
            float zero_point_center = (-static_cast<float>(v_min)) / (static_cast<float>(v_max) -
            ...
            if (v == 0.0f)
                ...
                result = (1.0f - static_cast<float>(ImLog(
                    -(FLOATTYPE)v_clamped / logarithmic_zero_epsilon) / ImLog(
            ...
            result = static_cast<float>(ImLog((FLOATTYPE)v_clamped / v_min_fudged) / ImLog(
                v_max_fudged / v_min_fudged));

        return flipped ? (1.0f - result) : result;
    }
    // Linear slider
    return static_cast<float>((FLOATTYPE)(SIGNEDTYPE)(v_clamped - v_min) / (FLOATTYPE)(SIGNEDTYPE)(
        v_max -
        v_min));
}
                                 ...
                                 float zero_deadzone_halfsize)
{
    ...
    if (is_logarithmic)
    {
        ...
        {
            float zero_point_center = (-static_cast<float>(ImMin(v_min, v_max))) / ImAbs(
                static_cast<float>(v_max) - static_cast<float>(v_min));
            // The zero point in parametric space
            float zero_point_snap_L = zero_point_center - zero_deadzone_halfsize;
            float zero_point_snap_R = zero_point_center + zero_deadzone_halfsize;
            if (t_with_flip >= zero_point_snap_L && t_with_flip <= zero_point_snap_R)
                result = static_cast<TYPE>(0.0f);
                // Special case to make getting exactly zero possible (the epsilon prevents it otherwise)
            else if (t_with_flip < zero_point_center)
                result = static_cast<TYPE>(-(logarithmic_zero_epsilon * ImPow(
                    -v_min_fudged / logarithmic_zero_epsilon,
                            ...
                            ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    const bool is_floating_point = (data_type == ImGuiDataType_Float) || (data_type ==
        ImGuiDataType_Double);
    const float v_range_f = static_cast<float>(v_min < v_max ? v_max - v_min : v_min - v_max);
    // We don't need high precision for what we do with it.
    ...
    if (g.ActiveId == id)
    {
        ...
        if (g.ActiveIdSource == ImGuiInputSource_Mouse)
        {
            ...
            else
            {
                ...
                if (g.ActiveIdIsJustActivated)
                {
                    ...
                    if (axis == ImGuiAxis_Y)
                        grab_t = 1.0f - grab_t;
                    const float grab_pos = ImLerp(slider_usable_pos_min, slider_usable_pos_max,
                                                  grab_t);
                    const bool clicked_around_grab = (mouse_abs_pos >= grab_pos - grab_sz * 0.5f -
                        1.0f) && (mouse_abs_pos <= grab_pos + grab_sz * 0.5f + 1.0f);
            ...
            ImGuiInputSource_Gamepad)
        {
            ...
            else if (g.SliderCurrentAccumDirty)
            {
                ...
                // This is to avoid applying the saturation when already past the limits
                {
                    ...
                    // If pushing up against the limits, don't continue to accumulate
                }
                else
                {
                    set_new_value = true;
        ...
        if (set_new_value)
            if ((g.LastItemData.InFlags & ImGuiItemFlags_ReadOnly) || (flags &
                           ...
                           ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    case ImGuiDataType_S8:
        {
            ...
            bool r = SliderBehaviorT<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, &v32,
                                                          *static_cast<const ImS8*>(p_min),
                                                          *static_cast<const ImS8*>(p_max), format,
                                                          flags, out_grab_bb);
            if (r) *static_cast<ImS8*>(p_v) = static_cast<ImS8>(v32);
            return r;
        }
    case ImGuiDataType_U8:
        {
            ...
            bool r = SliderBehaviorT<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32,
                                                          *static_cast<const ImU8*>(p_min),
                                                          *static_cast<const ImU8*>(p_max), format,
                                                          flags, out_grab_bb);
            if (r) *static_cast<ImU8*>(p_v) = static_cast<ImU8>(v32);
            return r;
        }
    case ImGuiDataType_S16:
        {
            ...
            bool r = SliderBehaviorT<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, &v32,
                                                          *static_cast<const ImS16*>(p_min),
                                                          *static_cast<const ImS16*>(p_max), format,
                                                          flags, out_grab_bb);
            if (r) *static_cast<ImS16*>(p_v) = static_cast<ImS16>(v32);
            return r;
        }
    case ImGuiDataType_U16:
        {
            ...
            bool r = SliderBehaviorT<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32,
                                                          *static_cast<const ImU16*>(p_min),
                                                          *static_cast<const ImU16*>(p_max), format,
                                                          flags, out_grab_bb);
            if (r) *static_cast<ImU16*>(p_v) = static_cast<ImU16>(v32);
            return r;
        }
    case ImGuiDataType_S32:
        IM_ASSERT(
            *static_cast<const ImS32*>(p_min)>= IM_S32_MIN / 2 && *static_cast<const ImS32*>(p_max)
            <= IM_S32_MAX / 2);
        return SliderBehaviorT<ImS32, ImS32, float>(bb, id, data_type, static_cast<ImS32*>(p_v),
                                                    *static_cast<const ImS32*>(p_min),
                                                    *static_cast<const ImS32*>(p_max), format,
                                                    flags, out_grab_bb);
    case ImGuiDataType_U32:
        IM_ASSERT(*static_cast<const ImU32*>(p_max)<= IM_U32_MAX / 2);
        return SliderBehaviorT<ImU32, ImS32, float>(bb, id, data_type, static_cast<ImU32*>(p_v),
                                                    *static_cast<const ImU32*>(p_min),
                                                    *static_cast<const ImU32*>(p_max), format,
                                                    flags, out_grab_bb);
    case ImGuiDataType_S64:
        IM_ASSERT(
            *static_cast<const ImS64*>(p_min)>= IM_S64_MIN / 2 && *static_cast<const ImS64*>(p_max)
            <= IM_S64_MAX / 2);
        return SliderBehaviorT<ImS64, ImS64, double>(bb, id, data_type, static_cast<ImS64*>(p_v),
                                                     *static_cast<const ImS64*>(p_min),
                                                     *static_cast<const ImS64*>(p_max), format,
                                                     flags, out_grab_bb);
    case ImGuiDataType_U64:
        IM_ASSERT(*static_cast<const ImU64*>(p_max)<= IM_U64_MAX / 2);
        return SliderBehaviorT<ImU64, ImS64, double>(bb, id, data_type, static_cast<ImU64*>(p_v),
                                                     *static_cast<const ImU64*>(p_min),
                                                     *static_cast<const ImU64*>(p_max), format,
                                                     flags, out_grab_bb);
    case ImGuiDataType_Float:
        IM_ASSERT(
            *static_cast<const float*>(p_min) >= -FLT_MAX / 2.0f && *static_cast<const float*>(p_max
            ) <= FLT_MAX / 2.0f);
        return SliderBehaviorT<float, float, float>(bb, id, data_type, static_cast<float*>(p_v),
                                                    *static_cast<const float*>(p_min),
                                                    *static_cast<const float*>(p_max), format,
                                                    flags, out_grab_bb);
    case ImGuiDataType_Double:
        IM_ASSERT(
            *static_cast<const double*>(p_min) >= -DBL_MAX / 2.0f && *static_cast<const double*>(
                p_max) <= DBL_MAX / 2.0f);
        return SliderBehaviorT<double, double, double>(bb, id, data_type, static_cast<double*>(p_v),
                                                       *static_cast<const double*>(p_min),
                                                       *static_cast<const double*>(p_max), format,
                                                       flags, out_grab_bb);
    case ImGuiDataType_COUNT: break;
    }
    IM_ASSERT(0);
                         ...
                         ImGuiSliderFlags flags)
{
    ...
    if (!temp_input_is_active)
    {
        ...
        if (make_active && clicked)
            SetKeyOwner(ImGuiKey_MouseLeft, id);
        if (make_active && temp_input_allowed)
            if ((clicked && g.IO.KeyCtrl) || (g.NavActivateId == id && (g.NavActivateFlags &
                          ...
                          ImGuiSliderFlags flags)
{
    ...
    RenderTextClipped(ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y), frame_bb.Max,
                      value_buf, value_buf_end, nullptr, ImVec2(0.5f, 0.0f));
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x,
                          frame_bb.Min.y + style.FramePadding.y), label);
...
const char* ImParseFormatFindEnd(const char* fmt)
{
    ...
    constexpr unsigned int ignored_lowercase_mask = (1 << ('h' - 'a')) | (1 << ('j' - 'a')) | (1 <<
        ('l' - 'a')) | (1 << ('t' - 'a')) | (1 << ('w' - 'a')) | (1 << ('z' - 'a'));
...
void ImParseFormatSanitizeForPrinting(const char* fmt_in, char* fmt_out, size_t fmt_out_size)
{
    ...
    IM_ASSERT(static_cast<size_t>(fmt_end - fmt_in + 1)< fmt_out_size);
                          ...
                          ImGuiInputTextFlags flags)
{
    ...
    bool value_changed = InputTextEx(label, nullptr, buf, buf_size, bb.GetSize(),
                            ...
                            const void* p_clamp_min, const void* p_clamp_max)
{
    ...
    ImGuiInputTextFlags flags = ImGuiInputTextFlags_AutoSelectAll | static_cast<ImGuiInputTextFlags>
    ...
    if (TempInputText(bb, id, label, data_buf, IM_ARRAYSIZE(data_buf), flags))
    {
        ...
        {
            if (p_clamp_min && p_clamp_max && DataTypeCompare(data_type, p_clamp_min, p_clamp_max) >
                0)
                        ...
                        ImGuiInputTextFlags flags)
{
    ...
    flags |= ImGuiInputTextFlags_AutoSelectAll | static_cast<ImGuiInputTextFlags>(
        ImGuiInputTextFlags_NoMarkEdited);
...
bool ImGui::InputFloat(const char* label, float* v, float step, float step_fast, const char* format,
                       ImGuiInputTextFlags flags)
{
    return InputScalar(label, ImGuiDataType_Float, v, step > 0.0f ? &step : nullptr,
                       step_fast > 0.0f ? &step_fast : nullptr, format, flags);
}
...
bool ImGui::InputInt(const char* label, int* v, int step, int step_fast, ImGuiInputTextFlags flags)
{
    ...
    return InputScalar(label, ImGuiDataType_S32, v, step > 0 ? &step : nullptr,
                       step_fast > 0 ? &step_fast : nullptr, format, flags);
}
...
{
    return InputScalar(label, ImGuiDataType_Double, v, step > 0.0 ? &step : nullptr,
                      ...
                      ImGuiInputTextCallback callback, void* user_data)
{
    ...
    return InputTextEx(label, nullptr, buf, static_cast<int>(buf_size), ImVec2(0, 0), flags,
                       callback, user_data);
}
...
bool ImGui::InputTextMultiline(const char* label, char* buf, size_t buf_size, const ImVec2& size,
                               ...
                               void* user_data)
{
    return InputTextEx(label, nullptr, buf, static_cast<int>(buf_size), size,
                              ...
                              void* user_data)
{
    ...
    return InputTextEx(label, hint, buf, static_cast<int>(buf_size), ImVec2(0, 0), flags, callback,
                       user_data);
}
...
namespace ImStb
{
                                       ...
                                       int line_start_idx)
    {
        ...
        const ImVec2 size = InputTextCalcTextSizeW(obj->Ctx, text + line_start_idx,
                                                   text + obj->CurLenW, &text_remaining, nullptr,
                                                   true);
        r->x0 = 0.0f;
        r->x1 = size.x;
    ...
    static int is_word_boundary_from_right(ImGuiInputTextState* obj, int idx)
    {
        ...
        return ((prev_white || prev_separ) && !(curr_separ || curr_white)) || (curr_separ && !
            prev_separ);
    }
                                         ...
                                         int new_text_len)
    {
        ...
        if (pos != text_len)
            memmove(text + pos + new_text_len, text + pos,
                    static_cast<size_t>(text_len - pos) * sizeof(ImWchar));
                                             ...
                                             const char* new_text_end)
{
    ...
    const int new_text_len = new_text_end
                                 ...
                                 : static_cast<int>(strlen(new_text));
                                     ...
                                     void* user_data, ImGuiInputSource input_source)
{
    ...
    if (c < 0x20)
    {
        ...
        // Override named filters below so newline and tabs can still be inserted.
    }
        ...
        ImGuiInputTextFlags_CharsNoBlank | ImGuiInputTextFlags_CharsScientific)))
    {
        ...
        ImGuiContext& g = *ctx;
        const unsigned c_decimal_point = g.IO.PlatformLocaleDecimalPoint;
                                                         ...
                                                         const char* new_buf_a, int new_length_a)
{
    ...
    auto new_buf = static_cast<ImWchar*>((void*)g.TempBuffer.Data);
    ...
    for (; old_last_diff >= first_diff && new_last_diff >= first_diff; old_last_diff--,
           new_last_diff--)
...
void ImGui::InputTextDeactivateHook(ImGuiID id)
{
    ...
    if (state->Flags & ImGuiInputTextFlags_ReadOnly)
    {
        ...
        // In theory this data won't be used, but clear to be neat.
    }
    else
    {
                        ...
                        ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    IM_ASSERT(
        !((flags & ImGuiInputTextFlags_CallbackCompletion) && (flags &
    ...
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth(),
                                           (is_multiline ? g.FontSize * 8.0f : label_size.y) + style
                                           .FramePadding.y * 2.0f);
    ...
    if (is_multiline)
    {
        ...
        if (g.NavActivateId == id && (g.NavActivateFlags & ImGuiActivateFlags_FromTabbing) && (flags
    ...
    const bool input_requested_by_nav = (g.ActiveId != id) && ((g.NavActivateId == id) && ((g.
        NavActivateFlags & ImGuiActivateFlags_PreferInput) || (g.NavInputSource ==
        ImGuiInputSource_Keyboard)));

    const bool user_clicked = hovered && io.MouseClicked[0];
    const bool user_scroll_finish = is_multiline && state != nullptr && g.ActiveId == 0 && g.
        ActiveIdPreviousFrame == GetWindowScrollbarID(draw_window, ImGuiAxis_Y);
    const bool user_scroll_active = is_multiline && state != nullptr && g.ActiveId ==
    ...
    // state != NULL means its our state.
    ...
    if ((init_state && g.ActiveId != id) || init_changed_specs)
    {
        ...
        if (!is_multiline)
        {
            if (flags & ImGuiInputTextFlags_AutoSelectAll)
                select_all = true;
            if (input_requested_by_nav && (!recycle_state || !(g.NavActivateFlags &
    ...
    if (is_readonly && state != nullptr && (render_cursor || render_selection))
    {
        const char* buf_end = nullptr;
        state->TextW.resize(buf_size + 1);
        state->CurLenW = ImTextStrFromUtf8(state->TextW.Data, state->TextW.Size, buf, nullptr,
                                           &buf_end);
        state->CurLenA = static_cast<int>(buf_end - buf);
        state->CursorClamp();
    ...
    const bool is_displaying_hint = (hint != nullptr && (buf_display_from_state
                                                             ? state->TextA.Data
                                                             : buf)[0] == 0);
    ...
    if (is_password && !is_displaying_hint)
    {
        ...
        IM_ASSERT(
            password_font->Glyphs.empty() && password_font->IndexAdvanceX.empty() && password_font->
            IndexLookup.empty());
    ...
    if (g.ActiveId == id)
    {
        ...
        // Edit in progress
        const float mouse_x = (io.MousePos.x - frame_bb.Min.x - style.FramePadding.x) + state->
            ScrollX;
        ...
        else if (io.MouseDown[0] && !state->SelectedAllMouseLock && (io.MouseDelta.x != 0.0f || io.
            MouseDelta.y != 0.0f))
        {
            stb_textedit_drag(state, &state->Stb, mouse_x, mouse_y);
            state->CursorAnimReset();
        ...
        if ((flags & ImGuiInputTextFlags_AllowTabInput) && Shortcut(
            ImGuiKey_Tab, id, ImGuiInputFlags_Repeat) && !is_readonly)
        {
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const int row_count_per_page = ImMax(
            static_cast<int>((inner_size.y - style.FramePadding.y) / g.FontSize), 1);
        ...
        const bool is_copy = (Shortcut(ImGuiMod_Shortcut | ImGuiKey_C, id) ||
            Shortcut(ImGuiMod_Ctrl | ImGuiKey_Insert, id)) && !is_password && (!is_multiline ||
            state->HasSelection());
        ...
        const bool is_redo = (Shortcut(ImGuiMod_Shortcut | ImGuiKey_Y, id, f_repeat) || (is_osx &&
                Shortcut(ImGuiMod_Shortcut | ImGuiMod_Shift | ImGuiKey_Z, id, f_repeat))) && !
            is_readonly && is_undoable;
        ...
        const bool is_enter_pressed = IsKeyPressed(ImGuiKey_Enter, true) || IsKeyPressed(
        ...
        const bool is_cancel = Shortcut(ImGuiKey_Escape, id, f_repeat) || (nav_gamepad_active &&
        ...
        {
            if (io.KeyCtrl) SetScrollY(draw_window,
                                       ImMax(draw_window->Scroll.y - g.FontSize, 0.0f));
            else state->OnKeyPressed(
        ...
        else if (is_cut || is_copy)
        {
            ...
            if (io.SetClipboardTextFn)
            {
                ...
                auto clipboard_data = static_cast<char*>(
                    IM_ALLOC(clipboard_data_len * sizeof(char)));
        ...
        else if (is_paste)
        {
            if (const char* clipboard = GetClipboardText())
            {
                ...
                auto clipboard_filtered = static_cast<ImWchar*>(IM_ALLOC(
    ...
    if (g.ActiveId == id)
    {
                ...
                if (apply_new_text_length > 0)
                {
                    w_text.resize(
                        ImTextCountCharsFromUtf8(apply_new_text,
                                                 apply_new_text + apply_new_text_length) + 1);
        ...
        const bool apply_edit_back_to_user_buffer = !revert_edit || (validated && (flags &
        ...
        if (apply_edit_back_to_user_buffer)
        {
                ...
                ImGuiInputTextFlags_CallbackAlways)) != 0)
            {
                ...
                if ((flags & ImGuiInputTextFlags_CallbackCompletion) != 0 && Shortcut(
                    ImGuiKey_Tab, id))
                {
                ...
                if (event_flag)
                {
                    ...
                    {
                        state->Stb.select_start = (callback_data.SelectionStart == callback_data.
                                                      CursorPos)
                                                      ? state->Stb.cursor
                    ...
                    {
                        state->Stb.select_end = (callback_data.SelectionEnd == callback_data.
                                                    SelectionStart)
                    ...
                    if (buf_dirty)
                    {
                        ...
                        IM_ASSERT(
                            callback_data.BufTextLen == static_cast<int>(strlen(callback_data.Buf)
    ...
    {
        if (g.ActiveId != id && IsItemDeactivatedAfterEdit() && !is_readonly && strcmp(
    ...
    if (render_cursor || render_selection)
    {
...

        {
            ...
            cursor_offset.x = InputTextCalcTextSizeW(
                &g, ImStrbolW(searches_input_ptr[0], text_begin), searches_input_ptr[0]).x;
            cursor_offset.y = searches_result_line_no[0] * g.FontSize;
            if (searches_result_line_no[1] >= 0)
            {
                select_start_offset.x = InputTextCalcTextSizeW(
                    &g, ImStrbolW(searches_input_ptr[1], text_begin), searches_input_ptr[1]).x;
        ...
        if (render_selection)
        {
            const ImWchar* text_selected_begin = text_begin + ImMin(
                state->Stb.select_start, state->Stb.select_end);
            const ImWchar* text_selected_end = text_begin + ImMax(
                state->Stb.select_start, state->Stb.select_end);
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end;)
            {
                ...
                else
                {
                    ImVec2 rect_size = InputTextCalcTextSizeW(
                        &g, p, text_selected_end, &p, nullptr, true);
                    if (rect_size.x <= 0.0f) rect_size.x = IM_TRUNC(
                        g.Font->GetCharAdvance(' ') * 0.50f); // So we can see selected empty lines
        ...
        if (render_cursor)
        {
            ...
            bool cursor_is_visible = (!g.IO.ConfigInputTextCursorBlink) || (state->CursorAnim <=
                0.0f) || ImFmod(state->CursorAnim, 1.20f) <= 0.80f;
    ...
    else
    {
        ...
        if (is_multiline)
            text_size = ImVec2(inner_size.x,
                               InputTextCalcTextLenAndLineCount(buf_display, &buf_display_end) * g.
                               FontSize); // We don't need width
...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState* state)
{
                   ...
                   ImGuiChildFlags_Border | ImGuiChildFlags_ResizeY)) // Visualize undo state
    {
        ...
        for (int n = 0; n < IMSTB_TEXTEDIT_UNDOSTATECOUNT; n++)
        {
            ...
            const char undo_rec_type = (n < undo_state->undo_point)
                                           ? 'u'
                                           : (n >= undo_state->redo_point)
                                           ? 'r'
                                           : ' ';
            if (undo_rec_type == ' ')
                BeginDisabled();
            ...
            if (undo_rec_type != ' ' && undo_rec->char_storage != -1)
                ImTextStrToUtf8(buf, IM_ARRAYSIZE(buf),
                                ...
                                undo_state->undo_char + undo_rec->char_storage + undo_rec->
                                insert_length);
...
static void ColorEditRestoreH(const float* col, float* H)
{
    ...
    if (g.ColorEditSavedID != g.ColorEditCurrentID || g.ColorEditSavedColor !=
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
        ...
        ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        const bool hide_prefix = (IM_TRUNC(w_items / components) <= CalcTextSize(
    ...
    else if ((flags & ImGuiColorEditFlags_DisplayHex) != 0 && (flags & ImGuiColorEditFlags_NoInputs)
        == 0)
    {
        ...
        if (InputText("##Text", buf, IM_ARRAYSIZE(buf), ImGuiInputTextFlags_CharsUppercase))
        {
            ...
            i[0] = i[1] = i[2] = 0;
            i[3] = 0xFF;
            // alpha default to 255 is not parsed by scanf (e.g. inputting #FFFFFF omitting alpha)
            int r;
            if (alpha)
    ...
    {
        const float button_offset_x = ((flags & ImGuiColorEditFlags_NoInputs) || (style.
    ...
    if ((g.LastItemData.StatusFlags & ImGuiItemStatusFlags_HoveredRect) && !(g.LastItemData.InFlags
            & ImGuiItemFlags_ReadOnly) && !(flags & ImGuiColorEditFlags_NoDragDrop) &&
        BeginDragDropTarget())
    {
                         ...
                         const float* ref_col)
{
    ...
    bool alpha_bar = (flags & ImGuiColorEditFlags_AlphaBar) && !(flags &
        ImGuiColorEditFlags_NoAlpha);
    ImVec2 picker_pos = window->DC.CursorPos;
    float square_sz = GetFrameHeight();
    float bars_width = square_sz; // Arbitrary smallish width of Hue/Alpha picking bars
    float sv_picker_size = ImMax(bars_width * 1,
                                 width - (alpha_bar ? 2 : 1) * (bars_width + style.ItemInnerSpacing.
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        if (IsItemActive() && !is_readonly)
        {
            ...
            if (initial_dist2 >= (wheel_r_inner - 1) * (wheel_r_inner - 1) && initial_dist2 <= (
    ...
    if (!(flags & ImGuiColorEditFlags_NoSidePreview))
    {
        ...
        ImGuiColorEditFlags sub_flags_to_forward = ImGuiColorEditFlags_InputMask_ |
    ...
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        ImGuiColorEditFlags sub_flags_to_forward = ImGuiColorEditFlags_DataTypeMask_ |
        ...
        if (flags & ImGuiColorEditFlags_DisplayRGB || (flags & ImGuiColorEditFlags_DisplayMask_) ==
            0)
        ...
        if (flags & ImGuiColorEditFlags_DisplayHSV || (flags & ImGuiColorEditFlags_DisplayMask_) ==
            0)
            value_changed |= ColorEdit4("##hsv", col, sub_flags | ImGuiColorEditFlags_DisplayHSV);
        if (flags & ImGuiColorEditFlags_DisplayHex || (flags & ImGuiColorEditFlags_DisplayMask_) ==
            0)
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        // Lock segment count so the +1 one matches others.
                        ...
                        const ImVec2& size_arg)
{
    ...
    // Tooltip
    if (!(flags & ImGuiColorEditFlags_NoTooltip) && hovered && IsItemHovered(
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ...
    int cr = IM_F32_TO_INT8_SAT(col[0]), cg = IM_F32_TO_INT8_SAT(col[1]), cb =
            IM_F32_TO_INT8_SAT(col[2]), ca = (flags & ImGuiColorEditFlags_NoAlpha)
                                                 ? 255
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
    ...
    bool allow_opt_alpha_bar = !(flags & ImGuiColorEditFlags_NoAlpha) && !(flags &
        ImGuiColorEditFlags_AlphaBar);
    ...
    if (allow_opt_picker)
    {
        ...
        for (int picker_type = 0; picker_type < 2; picker_type++)
        {
            ...
            PushID(picker_type);
            ImGuiColorEditFlags picker_flags = ImGuiColorEditFlags_NoInputs |
...
bool ImGui::TreeNodeUpdateNextOpen(ImGuiID id, ImGuiTreeNodeFlags flags)
{
    ...
    if (g.LogEnabled && !(flags & ImGuiTreeNodeFlags_NoAutoOpenOnLog) && (window->DC.TreeDepth - g.
        LogDepthRef) < g.LogDepthToExpand)
        is_open = true;
                             ...
                             const char* label_end)
{
    ...
    const bool span_all_columns = (flags & ImGuiTreeNodeFlags_SpanAllColumns) != 0 && (g.
    ...
    const float text_width = g.FontSize + (
        label_size.x > 0.0f ? label_size.x + padding.x * 2 : 0.0f); // Include collapsing
    ...
    if (!display_frame && (flags & (ImGuiTreeNodeFlags_SpanAvailWidth |
        ImGuiTreeNodeFlags_SpanFullWidth | ImGuiTreeNodeFlags_SpanAllColumns)) == 0)
    ...
    if (!item_add)
    {
        ...
        IMGUI_TEST_ENGINE_ITEM_INFO(g.LastItemData.ID, label,
                                    g.LastItemData.StatusFlags | (is_leaf ? 0 :
                                        ImGuiItemStatusFlags_Openable) | (is_open ?
    ...
    if ((flags & ImGuiTreeNodeFlags_AllowOverlap) || (g.LastItemData.InFlags &
    ...
    const bool is_mouse_x_over_arrow = (g.IO.MousePos.x >= arrow_hit_x1 && g.IO.MousePos.x <
        arrow_hit_x2);
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet)
            ...
            RenderArrow(window->DrawList,
                        ImVec2(text_pos.x - text_offset_x + padding.x,
                               text_pos.y + g.FontSize * 0.15f), text_col,
                        is_open
                            ? ((flags & ImGuiTreeNodeFlags_UpsideDownArrow)
                                   ? ImGuiDir_Up
                                   : ImGuiDir_Down)
...
bool ImGui::CollapsingHeader(const char* label, bool* p_visible, ImGuiTreeNodeFlags flags)
{
    ...
    if (p_visible)
        flags |= ImGuiTreeNodeFlags_AllowOverlap | static_cast<ImGuiTreeNodeFlags>(
                       ...
                       const ImVec2& size_arg)
{
    ...
    const bool item_add = ItemAdd(bb, id, nullptr,
    ...
    if ((flags & ImGuiSelectableFlags_AllowOverlap) || (g.LastItemData.InFlags &
        ImGuiItemFlags_AllowOverlap)) { button_flags |= ImGuiButtonFlags_AllowOverlap; }
    ...
    if ((flags & ImGuiSelectableFlags_SelectOnNav) && g.NavJustMovedToId != 0 && g.
        NavJustMovedToFocusScopeId == g.CurrentFocusScopeId)
    ...
    {
        if (!g.NavDisableMouseHover && g.NavWindow == window && g.NavLayer == window->DC.
            NavLayerCurrent)
        {
    ...
    // Automatically close popups
    if (pressed && (window->Flags & ImGuiWindowFlags_Popup) && !(flags &
        ImGuiSelectableFlags_DontClosePopups) && !(g.LastItemData.InFlags &
...
ImGuiTypingSelectRequest* ImGui::GetTypingSelectRequest(ImGuiTypingSelectFlags flags)
{
    ...
    // Clear buffer
    constexpr float TYPING_SELECT_RESET_TIMER = 1.80f; // FIXME: Potentially move to IO config.
    constexpr int TYPING_SELECT_SINGLE_CHAR_COUNT_FOR_LOCK = 4;
    // Lock single char matching when repeating same char 4 times
    if (data->SearchBuffer[0] != 0)
    {
        ...
        clear_buffer |= IsKeyPressed(ImGuiKey_Backspace) && (flags &
            ImGuiTypingSelectFlags_AllowBackspace) == 0;
    ...
    // Append to buffer
    constexpr int buffer_max_len = IM_ARRAYSIZE(data->SearchBuffer) - 1;
    ...
    for (ImWchar w : g.IO.InputQueueCharacters)
    {
        ...
        if (w < 32 || (buffer_len == 0 && ImCharIsBlankW(w)) || (buffer_len + w_len >
            buffer_max_len)) // Ignore leading blanks
            continue;
        char w_buf[5];
        ImTextCharToUtf8(w_buf, w);
        if (data->SingleCharModeLock && w_len == out_request->SingleCharSize && memcmp(
            w_buf, data->SearchBuffer, w_len) == 0)
        {
    ...
    // Handle backspace
    if ((flags & ImGuiTypingSelectFlags_AllowBackspace) && IsKeyPressed(
        ImGuiKey_Backspace, 0, ImGuiInputFlags_Repeat))
    {
        auto p = static_cast<char*>((void*)ImTextFindPreviousUtf8Codepoint(data->SearchBuffer,
    ...
    if (flags & ImGuiTypingSelectFlags_AllowSingleCharMode)
    {
        ...
        // From now on we stop search matching to lock to single char mode.
    }
...
int ImGui::TypingSelectFindMatch(ImGuiTypingSelectRequest* req, int items_count,
                                 ...
                                 int nav_item_idx)
{
    if (req == nullptr || req->SelectRequest == false)
                                            ...
                                            void* user_data)
{
    ...
    {
        const char* item_name = get_item_name_func(user_data, idx);
        const int match_len = ImStrimatchlen(req->SearchBuffer,
...
bool ImGui::BeginListBox(const char* label, const ImVec2& size_arg)
{
    ...
    ImVec2 size = ImTrunc(CalcItemSize(size_arg, CalcItemWidth(),
                                       GetTextLineHeightWithSpacing() * 7.25f + style.FramePadding.y
                                       * 2.0f));
    ...
    if (label_size.x > 0.0f)
    {
        auto label_pos = ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x,
...
void ImGui::EndListBox()
{
    ...
    EndChild();
    EndGroup();
    // This is only required to be able to do IsItemXXX query on the whole ListBox including label
}
                    ...
                    int items_count, int height_in_items)
{
    ...
    ImVec2 size(0.0f, ImTrunc(
                    GetTextLineHeightWithSpacing() * height_in_items_f + g.Style.FramePadding.y *
                    2.0f));
                  ...
                  const ImVec2& size_arg)
{
    ...
    {
        int res_w = ImMin(static_cast<int>(frame_size.x), values_count) + (
        ...
        float t0 = 0.0f;
        auto tp0 = ImVec2(t0, 1.0f - ImSaturate((v0 - scale_min) * inv_scale));
...
static float Plot_ArrayGetter(void* data, int idx)
{
    ...
    const float v = *static_cast<const float*>((const void*)((const unsigned char*)plot_data->Values
        + (size_t)idx *
        plot_data->Stride));
    return v;
}
                      ...
                      int stride)
{
    ...
    PlotEx(ImGuiPlotType_Lines, label, &Plot_ArrayGetter, &data, values_count, values_offset,
...
{
    ImGuiPlotArrayGetterData data(values, stride);
    PlotEx(ImGuiPlotType_Histogram, label, &Plot_ArrayGetter, &data, values_count, values_offset,
           overlay_text, scale_min, scale_max, graph_size);
}
...
bool ImGui::BeginMenuBar()
{
    ...
    ImRect clip_rect(
        ...
        IM_ROUND(
            ImMax(bar_rect.Min.x, bar_rect.Max.x - ImMax(window->WindowRounding, window->
                WindowBorderSize))), IM_ROUND(bar_rect.Max.y));
...
void ImGui::EndMenuBar()
{
    ...
    if (NavMoveRequestButNoResultYet() && (g.NavMoveDir == ImGuiDir_Left || g.NavMoveDir ==
        ImGuiDir_Right) && (g.NavWindow->Flags & ImGuiWindowFlags_ChildMenu))
    {
            ...
            ImGuiLayoutType_Horizontal && (g.NavMoveFlags & ImGuiNavMoveFlags_Forwarded) == 0)
        {
            ...
            // Sanity check (FIXME: Seems unnecessary)
            FocusWindow(window);
    ...
    // Convert ideal extents for scrolling layer equivalent.
    EndGroup();
    // Restore position on layer 0 // FIXME: Misleading to use a group for that backup/restore
    window->DC.LayoutType = ImGuiLayoutType_Vertical;
    window->DC.IsSameLine = false;
                                 ...
                                 float axis_size, ImGuiWindowFlags window_flags)
{
    ...
    auto viewport = static_cast<ImGuiViewportP*>((void*)(
        viewport_p ? viewport_p : GetMainViewport()));
    ...
    // Enforce viewport so we don't create our own viewport when ImGuiConfigFlags_ViewportsNoMerge is set.
...
bool ImGui::BeginMainMenuBar()
{
    ...
    SetCurrentViewport(nullptr, viewport);
    ...
    g.NextWindowData.MenuBarOffsetMinVal = ImVec2(g.Style.DisplaySafeAreaPadding.x,
                                                  ImMax(
                                                      g.Style.DisplaySafeAreaPadding.y - g.Style.
                                                      FramePadding.y, 0.0f));
    ImGuiWindowFlags window_flags = ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoSavedSettings
        | ImGuiWindowFlags_MenuBar;
    float height = GetFrameHeight();
    bool is_open = BeginViewportSideBar("##MainMenuBar", viewport, ImGuiDir_Up, height,
                                        window_flags);
...
void ImGui::EndMainMenuBar()
{
    ...
    if (g.CurrentWindow == g.NavWindow && g.NavLayer == ImGuiNavLayer_Main && !g.NavAnyRequest)
        FocusTopMostWindowUnderOne(g.NavWindow, nullptr, nullptr,
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    constexpr ImGuiSelectableFlags selectable_flags = ImGuiSelectableFlags_NoHoldingActiveID |
    ...
    // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        ImGuiPopupData* child_popup = (g.BeginPopupStack.Size < g.OpenPopupStack.Size)
                                          ...
                                          : nullptr; // Popup candidate (testing below)
        ImGuiWindow* child_menu_window = (child_popup && child_popup->Window && child_popup->Window
                                             ->ParentWindow == window)
                                             ? child_popup->Window
                                             : nullptr;
        ...
        if (menu_is_open && !hovered && g.HoveredWindow == window && !moving_toward_child_menu && !g
            .NavDisableMouseHover && g.ActiveId == 0)
            want_close = true;
        ...
        if (!menu_is_open && pressed) // Click/activate to open
            ...
            want_open = true;
        else if (!menu_is_open && hovered && g.HoveredIdTimer >= 0.30f && g.MouseStationaryTimer >=
...
void ImGui::EndMenu()
{
    ...
    // Should always be != NULL is we passed assert.
    if (window->BeginCount == window->BeginCountPreviousFrame)
        if (g.NavMoveDir == ImGuiDir_Left && NavMoveRequestButNoResultYet())
            if (g.NavWindow && (g.NavWindow->RootWindowForNav == window) && parent_window->DC.
                       ...
                       bool enabled)
{
    ...
    // We use ImGuiSelectableFlags_NoSetKeyOwner to allow down on one menu item, move, up on another.
    constexpr ImGuiSelectableFlags selectable_flags = ImGuiSelectableFlags_SelectOnRelease |
...
{
    return (tab->Flags & ImGuiTabItemFlags_Leading)
               ? 0
...
{
    auto a = static_cast<const ImGuiTabItem*>(lhs);
    auto b = static_cast<const ImGuiTabItem*>(rhs);
...
bool ImGui::BeginTabBar(const char* str_id, ImGuiTabBarFlags flags)
{
    ...
    auto tab_bar_bb = ImRect(window->DC.CursorPos.x, window->DC.CursorPos.y, window->WorkRect.Max.x,
...
{
    return tab_bar->BarRect.GetWidth() - sections[0].Width - sections[2].Width - sections[1].
        Spacing;
}
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    sections[0].Spacing = sections[0].TabCount > 0 && (sections[1].TabCount + sections[2].TabCount)
                          > 0
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        if ((most_recently_selected_tab == nullptr || most_recently_selected_tab->LastFrameSelected
            < tab->LastFrameSelected) && !(tab->Flags & ImGuiTabItemFlags_Button))
        ...
        const bool has_close_button_or_unsaved_marker = (tab->Flags &
            ImGuiTabItemFlags_NoCloseButton) == 0 || (tab->Flags &
        ...
        section->Width += tab->ContentWidth + (section_n == curr_section_n
                                                   ? g.Style.ItemInnerSpacing.x
                                                   : 0.0f);
        ...
        ;
        ImGuiShrinkWidthItem* shrink_width_item = &g.ShrinkWidthBuffer[shrink_buffer_indexes[
            section_n]++];
    ...
    for (int section_n = 0; section_n < 3; section_n++)
    {
        ImGuiTabBarSection* section = &sections[section_n];
        if (section_n == 2)
            tab_offset = ImMin(ImMax(0.0f, tab_bar->BarRect.GetWidth() - section->Width),
                               tab_offset);
    ...
    if (g.NavWindowingTarget != nullptr && g.NavWindowingTarget->DockNode && g.NavWindowingTarget->
        DockNode->TabBar == tab_bar)
        tab_bar->VisibleTabId = scroll_to_tab_id = g.NavWindowingTarget->TabId;

    // Apply request requests
    if (scroll_to_tab_id != 0)
        TabBarScrollToTab(tab_bar, scroll_to_tab_id, sections);
    else if ((tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyScroll) && IsMouseHoveringRect(
        tab_bar->BarRect.Min, tab_bar->BarRect.Max,
        true) && IsWindowContentHoverable(g.CurrentWindow))
    {
...
void ImGui::TabBarAddTab(ImGuiTabBar* tab_bar, ImGuiTabItemFlags tab_flags, ImGuiWindow* window)
{
    ...
    // Can't work while the tab bar is active as our tab doesn't have an X offset yet, in theory we could/should test something like (tab_bar->CurrFrameVisible < g.FrameCount) but we'd need to solve why triggers the commented early-out assert in BeginTabBarEx() (probably dock node going from implicit to explicit in same frame)
                                     ...
                                     ImGuiTabBarSection* sections)
{
    ...
    float tab_x1 = tab->Offset - sections[0].Width + (order > sections[0].TabCount - 1
                                                          ? -margin
                                                          : 0.0f);
                                           ...
                                           ImVec2 mouse_pos)
{
    ...
    const float bar_offset = tab_bar->BarRect.Min.x - (is_central_section
                                                           ? tab_bar->ScrollingTarget
                                                           : 0);
    ...
    for (int i = src_idx; i >= 0 && i < tab_bar->Tabs.Size; i += dir)
    {
        ...
        if (dst_tab->Flags & ImGuiTabItemFlags_NoReorder)
            break;
        if ((dst_tab->Flags & ImGuiTabItemFlags_SectionMask_) != (src_tab->Flags &
...
bool ImGui::TabBarProcessReorder(ImGuiTabBar* tab_bar)
{
    ...
    if (tab2->Flags & ImGuiTabItemFlags_NoReorder)
        return false;
    if ((tab1->Flags & ImGuiTabItemFlags_SectionMask_) != (tab2->Flags &
        ImGuiTabItemFlags_SectionMask_))
        return false;
...
static ImGuiTabItem* ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar)
{
    ...
    PushStyleColor(ImGuiCol_Text, arrow_col);
    PushStyleColor(ImGuiCol_Button, ImVec4(0, 0, 0, 0));
    bool open = BeginCombo("##v", nullptr,
...
bool ImGui::TabItemButton(const char* label, ImGuiTabItemFlags flags)
{
    ...
    return TabItemEx(tab_bar, label, nullptr,
                     flags | ImGuiTabItemFlags_Button | ImGuiTabItemFlags_NoReorder, nullptr);
}
                      ...
                      ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    // Can't use both Leading and Trailing
    ...
    ImVec2 size = TabItemCalcSize(
        label, (p_open != nullptr) || (flags & ImGuiTabItemFlags_UnsavedDocument));
    ...
    const bool tab_just_unsaved = (flags & ImGuiTabItemFlags_UnsavedDocument) && !(tab->Flags &
        ImGuiTabItemFlags_UnsavedDocument);
    ...
    if (!is_tab_button)
    {
        if (tab_appearing && (tab_bar->Flags & ImGuiTabBarFlags_AutoSelectNewTabs) && tab_bar->
            NextSelectedTabId == 0)
    ...
    if (!tab_contents_visible && tab_bar->SelectedTabId == 0 && tab_bar_appearing && docked_window
        == nullptr)
    ...
    if (is_central_section)
        window->DC.CursorPos = tab_bar->BarRect.Min + ImVec2(
            IM_TRUNC(tab->Offset - tab_bar->ScrollingAnim), 0.0f);
    else
    ...
    const bool want_clip_rect = is_central_section && (bb.Min.x < tab_bar->ScrollingRectMinX || bb.
        Max.x > tab_bar->ScrollingRectMaxX);
    if (want_clip_rect)
    ...
    const bool single_floating_window_node = node && node->IsFloatingNode() && (node->Windows.Size
        == 1);
    ...
    else if (held && !tab_appearing && IsMouseDragging(0))
    {
        ...
        if (!g.DragDropActive && ((tab_bar->Flags & ImGuiTabBarFlags_Reorderable) || (docked_window
            != nullptr)))
        {
        ...
        const bool can_undock = docked_window != nullptr && !(docked_window->Flags &
            ImGuiWindowFlags_NoMove) && !(node->MergedFlags & ImGuiDockNodeFlags_NoUndocking);
        if (can_undock)
        {
            ...
            if (!undocking_tab) //&& (!g.IO.ConfigDockingWithShift || g.IO.KeyShift)
            {
                ...
                float threshold_y = (threshold_base * 1.5f) + ImClamp(
                ...
                if (drag_distance_from_edge_x > threshold_x)
                    if ((drag_dir < 0 && TabBarGetTabOrder(tab_bar, tab) == 0) || (drag_dir > 0 &&
    ...
    const ImGuiID close_button_id = p_open
                                        ? GetIDWithSeed("#CLOSE", nullptr,
    ...
    TabItemLabelAndCloseButton(display_draw_list, bb,
                               tab_just_unsaved
                                   ...
                                   : flags, tab_bar->FramePadding, label, id, close_button_id,
    ...
    if (text_clipped && g.HoveredId == id && !held)
        if (!(tab_bar->Flags & ImGuiTabBarFlags_NoTooltip) && !(tab->Flags &
...
{
    ImGuiContext& g = *GImGui;
    bool is_within_manual_tab_bar = g.CurrentTabBar && !(g.CurrentTabBar->Flags &
...
ImVec2 ImGui::TabItemCalcSize(const char* label, bool has_close_button_or_unsaved_marker)
{
    ImGuiContext& g = *GImGui;
    ImVec2 label_size = CalcTextSize(label, nullptr, true);
    auto size = ImVec2(label_size.x + g.Style.FramePadding.x,
...
ImVec2 ImGui::TabItemCalcSize(ImGuiWindow* window)
{
    return TabItemCalcSize(window->Name,
                           window->HasCloseButton || (window->Flags &
                                       ...
                                       bool* out_text_clipped)
{
    ...
    if (close_button_id != 0)
        if (is_contents_visible || bb.GetWidth() >= ImMax(button_sz,
                                                          g.Style.TabMinWidthForCloseButton))
            if (g.HoveredId == tab_id || g.HoveredId == close_button_id || g.ActiveId == tab_id || g
                .ActiveId == close_button_id)
                close_button_visible = true;
    bool unsaved_marker_visible = (flags & ImGuiTabItemFlags_UnsavedDocument) != 0 && (button_pos.x
        + button_sz <= bb.Max.x);
    ...
    RenderTextEllipsis(draw_list, text_ellipsis_clip_bb.Min, text_ellipsis_clip_bb.Max,
                       text_pixel_clip_bb.Max.x, ellipsis_max_x, label, nullptr, &label_size);
