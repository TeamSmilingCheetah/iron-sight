...
// Temporary storage for last TreeNode() being a Left arrow landing candidate.
...
// -> enum ImGuiDataAuthority_      // Enum: for storing the source authority (dock node vs window) of a field
using ImGuiLayoutType = int; // -> enum ImGuiLayoutType_         // Enum: Horizontal or vertical
...
// -> enum ImGuiDebugLogFlags_      // Flags: for ShowDebugLogWindow(), g.DebugLogFlags
using ImGuiFocusRequestFlags = int; // -> enum ImGuiFocusRequestFlags_  // Flags: for FocusWindow();
...
// -> enum ImGuiItemFlags_          // Flags: for PushItemFlag(), g.LastItemData.InFlags
using ImGuiItemStatusFlags = int;
// -> enum ImGuiItemStatusFlags_    // Flags: for g.LastItemData.StatusFlags
using ImGuiOldColumnFlags = int; // -> enum ImGuiOldColumnFlags_     // Flags: for BeginColumns()
using ImGuiNavHighlightFlags = int;
// -> enum ImGuiNavHighlightFlags_  // Flags: for RenderNavHighlight()
using ImGuiNavMoveFlags = int; // -> enum ImGuiNavMoveFlags_       // Flags: for navigation requests
...
// -> enum ImGuiScrollFlags_        // Flags: for ScrollToItem() and navigation requests
using ImGuiSeparatorFlags = int; // -> enum ImGuiSeparatorFlags_     // Flags: for SeparatorEx()
using ImGuiTextFlags = int; // -> enum ImGuiTextFlags_          // Flags: for TextEx()
using ImGuiTooltipFlags = int; // -> enum ImGuiTooltipFlags_       // Flags: for BeginTooltipEx()
using ImGuiTypingSelectFlags = int;
// -> enum ImGuiTypingSelectFlags_  // Flags: for GetTypingSelectRequest()
...
static inline void ImQsort(void* base, size_t count, size_t size_of_element,
                           int (IMGUI_CDECL *compare_func)(const void*, const void*))
{
...
IMGUI_API int ImTextStrFromUtf8(ImWchar* out_buf, int out_buf_size, const char* in_text,
                                const char* in_text_end,
                                const char** in_remaining = nullptr);
...
IMGUI_API void* ImFileLoadToMemory(const char* filename, const char* mode,
                                   size_t* out_file_size = nullptr, int padding_bytes = 0);
...
static inline float ImTrunc(float f) { return static_cast<float>(static_cast<int>(f)); }

static inline ImVec2 ImTrunc(const ImVec2& v)
{
    return ImVec2(static_cast<float>(static_cast<int>(v.x)),
                  static_cast<float>(static_cast<int>(v.y)));
}

static inline float ImFloor(float f)
{
    return static_cast<float>((f >= 0 || static_cast<float>((int)f) == f)
                                  ? static_cast<int>(f)
                                  : static_cast<int>(f) - 1);
...
IMGUI_API ImVec2 ImBezierCubicClosestPoint(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3,
                                           const ImVec2& p4, const ImVec2& p,
                                           int num_segments);
// For curves with explicit number of segments
IMGUI_API ImVec2 ImBezierCubicClosestPointCasteljau(const ImVec2& p1, const ImVec2& p2,
...
struct ImVec2ih
{
    ...
    constexpr explicit ImVec2ih(const ImVec2& rhs) : x(static_cast<short>(rhs.x)),
                                                     y(static_cast<short>(rhs.y))
    {
    }
};
...
struct IMGUI_API ImRect
{
    ...
    {
        return p.x >= Min.x - pad.x && p.y >= Min.y - pad.y && p.x < Max.x + pad.x && p.y < Max.y +
            pad.y;
    }
...
{
    return static_cast<size_t>((bitcount + 31) >> 5) << 2;
}
...
{
    ImU32 mask = static_cast<ImU32>(1) << (n & 31);
...
inline void ImBitArraySetBitRange(ImU32* arr, int n, int n2) // Works on range [n..n2)
{
    ...
    while (n <= n2)
    {
        ...
        ImU32 mask = static_cast<ImU32>((static_cast<ImU64>(1) << b_mod) - 1) & ~static_cast<ImU32>(
            (static_cast<ImU64>(1) << a_mod) - 1);
        arr[n >> 5] |= mask;
        n = (n + 32) & ~31;
    }
}

using ImBitArrayPtr = ImU32*; // Name for use in structs
...
struct IMGUI_API ImBitVector
{
    ...
    void Create(int sz)
    {
        ...
        memset(Storage.Data, 0, static_cast<size_t>(Storage.Size) * sizeof(Storage.Data[0]));
    }
...
struct ImSpan
{
    ...
    int size() const { return static_cast<int>(static_cast<ptrdiff_t>(DataEnd - Data)); }

    int size_in_bytes() const
    {
        return static_cast<int>(static_cast<ptrdiff_t>(DataEnd - Data)) * static_cast<int>(sizeof(
            T));
    }
...
struct ImSpanAllocator
{
    ...
    void Reserve(int n, size_t sz, int a = 4)
    {
        ...
        Offsets[n] = CurrOff;
        Sizes[n] = static_cast<int>(sz);
        CurrIdx++;
        CurrOff += static_cast<int>(sz);
    }

    int GetArenaSizeInBytes() { return CurrOff; }
    void SetArenaBasePtr(void* base_ptr) { BasePtr = static_cast<char*>(base_ptr); }

    void* GetSpanPtrBegin(int n)
    {
        IM_ASSERT(n >= 0 && n < CHUNKS && CurrIdx == CHUNKS);
        return static_cast<void*>(BasePtr + Offsets[n]);
    }

    void* GetSpanPtrEnd(int n)
    {
        IM_ASSERT(n >= 0 && n < CHUNKS && CurrIdx == CHUNKS);
        return static_cast<void*>(BasePtr + Offsets[n] + Sizes[n]);
    }

    template <typename T>
    void GetSpan(int n, ImSpan<T>* span)
    {
        span->set(static_cast<T*>(GetSpanPtrBegin(n)), static_cast<T*>(GetSpanPtrEnd(n)));
    }
};
...
struct ImPool
{
    ...
    ImPoolIdx GetIndex(const T* p) const
    {
        ...
        return static_cast<ImPoolIdx>(p - Buf.Data);
    }
    ...
    T* Add()
    {
        ...
        else { FreeIdx = *static_cast<int*>(&Buf[idx]); }
    ...
    {
        Buf[idx].~T();
        *static_cast<int*>(&Buf[idx]) = FreeIdx;
        FreeIdx = idx;
        Map.SetInt(key, -1);
        AliveCount--;
    }
...
struct ImChunkStream
{
    ...
    T* alloc_chunk(size_t sz)
    {
        ...
        static_cast<int*>(static_cast<void*>(Buf.Data + off))[0] = static_cast<int>(sz);
        return static_cast<T*>(static_cast<void*>(Buf.Data + off + (int)HDR_SZ));
    }
    ...
    T* begin()
    {
        ...
        return static_cast<T*>(static_cast<void*>(Buf.Data + HDR_SZ));
    }
    ...
    T* next_chunk(T* p)
    {
        ...
        p = static_cast<T*>(static_cast<void*>((char*)(void*)p + chunk_size(p)));
        if (p == static_cast<T*>(static_cast<void*>((char*)end() + HDR_SZ))) return nullptr;
        IM_ASSERT(p < end());
        return p;
    }
    ...
    {
        IM_ASSERT(p >= begin() && p < end());
        const ptrdiff_t off = static_cast<const char*>(p) - Buf.Data;
        return static_cast<int>(off);
    }
    ...
    T* ptr_from_offset(int off)
    {
        ...
        return static_cast<T*>(static_cast<void*>(Buf.Data + off));
    }
...
enum ImGuiHoveredFlagsPrivate_
{
    ...
    ImGuiHoveredFlags_RootWindow | ImGuiHoveredFlags_AnyWindow | ImGuiHoveredFlags_NoPopupHierarchy
    | ImGuiHoveredFlags_DockHierarchy | ImGuiHoveredFlags_AllowWhenBlockedByPopup |
...
struct ImGuiDataVarInfo
{
    ...
    void* GetVarPtr(void* parent) const { return static_cast<unsigned char*>(parent) + Offset; }
};
...
struct ImGuiLastItemData
{
    ...
    ImRect DisplayRect;
    // Display rectangle (ONLY VALID IF ImGuiItemStatusFlags_HasDisplayRect is set)
    ImRect ClipRect;
    // Clip rectangle at the time of submitting item (ONLY VALID IF ImGuiItemStatusFlags_HasClipRect is set)
...
using ImBitArrayForNamedKeys = ImBitArray<ImGuiKey_NamedKey_COUNT, -ImGuiKey_NamedKey_BEGIN>;
...
enum ImGuiInputFlags_
{
    ...
    ImGuiInputFlags_RouteGlobalLow | ImGuiInputFlags_RouteGlobalHigh, // _Always not part of this!
    ...
    ImGuiInputFlags_RouteExtraMask_ = ImGuiInputFlags_RouteAlways |
    ...
    ImGuiInputFlags_RepeatUntilMask_ = ImGuiInputFlags_RepeatUntilRelease |
    ImGuiInputFlags_RepeatUntilKeyModsChange | ImGuiInputFlags_RepeatUntilKeyModsChangeFromNone |
    ...
    ImGuiInputFlags_SupportedBySetKeyOwner = ImGuiInputFlags_LockThisFrame |
...
struct ImGuiListClipperRange
{
    ...
    static ImGuiListClipperRange FromPositions(float y1, float y2, int off_min, int off_max)
    {
        ImGuiListClipperRange r = {
            static_cast<int>(y1), static_cast<int>(y2), true, static_cast<ImS8>(off_min),
            static_cast<ImS8>(off_max)
        };
        return r;
    }
};
...
struct ImGuiNavItemData
{
    ...
    ImGuiSelectionUserData SelectionUserData;
    //I+Mov    // Best candidate SetNextItemSelectionData() value.
...
enum ImGuiDockNodeFlagsPrivate_
{
    ...
    ImGuiDockNodeFlags_NoDockingSplit | ImGuiDockNodeFlags_NoDockingSplitOther,
    // Masks
    ...
    ImGuiDockNodeFlags_LocalFlagsTransferMask_ = ImGuiDockNodeFlags_NoDockingSplit |
...
struct IMGUI_API ImGuiDockNode
{
    ...
    {
        return ParentNode == nullptr && (MergedFlags & ImGuiDockNodeFlags_DockSpace) == 0;
    }
...
{
    ImGuiContextHookType_NewFramePre, ImGuiContextHookType_NewFramePost,
    ImGuiContextHookType_EndFramePre, ImGuiContextHookType_EndFramePost,
...
struct ImGuiContext
{
    ...
    // Stack for TreeNode() when a NavLeft requested is emitted.
...
namespace ImGui
{
    ...
    IMGUI_API bool MenuItemEx(const char* label, const char* icon, const char* shortcut = nullptr,
    ...
    {
        return (key >= ImGuiKey_NamedKey_BEGIN && key < ImGuiKey_NamedKey_END) || key ==
            ImGuiMod_Ctrl || key == ImGuiMod_Shift || key == ImGuiMod_Alt || key == ImGuiMod_Super
    ...
    inline ImGuiKeyChord ConvertShortcutMod(ImGuiKeyChord key_chord)
    {
        ...
        return (key_chord & ~ImGuiMod_Shortcut) | (g.IO.ConfigMacOSXBehaviors
                                                       ? ImGuiMod_Super
                                                       : ImGuiMod_Ctrl);
    }
    ...
    inline ImGuiKey ConvertSingleModFlagToKey(ImGuiContext* ctx, ImGuiKey key)
    {
        ...
        if (key == ImGuiMod_Shortcut) return (g.IO.ConfigMacOSXBehaviors
    ...
    inline ImGuiKey MouseButtonToKey(ImGuiMouseButton button)
    {
        ...
        return static_cast<ImGuiKey>(ImGuiKey_MouseLeft + button);
    }
    ...
    IMGUI_API ImGuiID DockBuilderSplitNode(ImGuiID node_id, ImGuiDir split_dir,
                                           float size_ratio_for_node_at_dir, ImGuiID* out_id_at_dir,
    ...
    IMGUI_API void RenderText(ImVec2 pos, const char* text, const char* text_end = nullptr,
    ...
    IMGUI_API const char* FindRenderedTextEnd(const char* text, const char* text_end = nullptr);
    ...
    // Widgets
    IMGUI_API void TextEx(const char* text, const char* text_end = nullptr,
    ...
    IMGUI_API bool TempInputScalar(const ImRect& bb, ImGuiID id, const char* label,
                                   ...
                                   const void* p_clamp_min = nullptr,
                                   const void* p_clamp_max = nullptr);
    ...
    IMGUI_API void ShadeVertsTransformPos(ImDrawList* draw_list, int vert_start_idx,
                                          int vert_end_idx, const ImVec2& pivot_in, float cos_a,
