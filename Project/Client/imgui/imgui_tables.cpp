...
static constexpr int TABLE_DRAW_CHANNEL_NOCLIP = 2;
// When using ImGuiTableFlags_NoClip (this becomes the last visible channel)
static constexpr float TABLE_BORDER_SIZE = 1.0f;
// FIXME-TABLE: Currently hard-coded because of clipping assumptions with outer borders rendering.
static constexpr float TABLE_RESIZE_SEPARATOR_HALF_THICKNESS = 4.0f;
// Extend outside inner borders.
static constexpr float TABLE_RESIZE_SEPARATOR_FEEDBACK_TIMER = 0.06f;
...
inline ImGuiTableFlags TableFixFlags(ImGuiTableFlags flags, ImGuiWindow* outer_window)
{
    ...
    if ((flags & ImGuiTableFlags_SizingMask_) == 0)
        flags |= ((flags & ImGuiTableFlags_ScrollX) || (outer_window->Flags &
                         ...
                         const ImVec2& outer_size, float inner_width)
{
    ...
    const bool use_child_window = (flags & (ImGuiTableFlags_ScrollX | ImGuiTableFlags_ScrollY)) !=
        0;
    const ImVec2 avail_size = GetContentRegionAvail();
    const ImVec2 actual_outer_size = CalcItemSize(outer_size, ImMax(avail_size.x, 1.0f),
                                                  use_child_window
                                                      ? ImMax(avail_size.y, 1.0f)
                                                      : 0.0f);
    const ImRect outer_rect(outer_window->DC.CursorPos,
                            outer_window->DC.CursorPos + actual_outer_size);
    const bool outer_window_is_measuring_size = (outer_window->AutoFitFramesX > 0) || (outer_window
        ->AutoFitFramesY > 0); // Doesn't apply to AlwaysAutoResize windows!
    ...
    ImGuiTableTempData* temp_data = table->TempData = &g.TablesTempData[g.TablesTempDataStacked -
        1];
    ...
    if (instance_no > 0)
    {
        ...
        instance_id = GetIDWithSeed(instance_no, GetIDWithSeed("##Instances", nullptr, id));
        // Push "##Instances" followed by (int)instance_no in ID stack.
    }
    else
    {
        instance_id = id;
    ...
    if (use_child_window)
    {
        ...
        IM_ASSERT(
            table->InnerWindow->WindowPadding.x == 0.0f && table->InnerWindow->WindowPadding.y ==
            0.0f && table->InnerWindow->WindowBorderSize == 0.0f);
    ...
    if (inner_window != outer_window)
    {
        if (flags & ImGuiTableFlags_BordersOuterV)
            table->HostClipRect.Min.x = ImMin(table->HostClipRect.Min.x + TABLE_BORDER_SIZE,
                                              table->HostClipRect.Max.x);
        if (flags & ImGuiTableFlags_BordersOuterH)
            table->HostClipRect.Min.y = ImMin(table->HostClipRect.Min.y + TABLE_BORDER_SIZE,
    ...
    const float inner_spacing_for_border = (flags & ImGuiTableFlags_BordersInnerV)
                                               ? TABLE_BORDER_SIZE
                                               : 0.0f;
    const float inner_spacing_explicit = (pad_inner_x && (flags & ImGuiTableFlags_BordersInnerV) ==
                                             0)
                                             ? g.Style.CellPadding.x
                                             : 0.0f;
    const float inner_padding_explicit = (pad_inner_x && (flags & ImGuiTableFlags_BordersInnerV) !=
                                             0)
                                             ? g.Style.CellPadding.x
                                             : 0.0f;
    ...
    const float outer_padding_for_border = (flags & ImGuiTableFlags_BordersOuterV)
                                               ? TABLE_BORDER_SIZE
                                               : 0.0f;
    const float outer_padding_explicit = pad_outer_x ? g.Style.CellPadding.x : 0.0f;
    table->OuterPaddingX = (outer_padding_for_border + outer_padding_explicit) - table->
        CellPaddingX;
    ...
    // This will be setup by TableSetupScrollFreeze(), if any
    ...
    // Shortcut for NavUpdateCurrentWindowIsScrollPushableX();
    ...
    if (table->IsInitializing)
    {
        ...
        for (int n = 0; n < columns_count; n++)
        {
            ...
            column->DisplayOrder = table->DisplayOrderToIndex[n] = static_cast<ImGuiTableColumnIdx>(
                n);
        }
    }
...
void ImGui::TableBeginInitMemory(ImGuiTable* table, int columns_count)
{
    ...
    const int columns_bit_array_size = static_cast<int>(ImBitArrayGetStorageSizeInBytes(
        columns_count));
...
void ImGui::TableBeginApplyRequests(ImGuiTable* table)
{
    ...
    if (table->InstanceCurrent == 0)
    {
        ...
        if (table->ReorderColumn != -1 && table->ReorderColumnDir != 0)
        {
            ...
            for (int order_n = src_order + reorder_dir; order_n != dst_order + reorder_dir; order_n
                 += reorder_dir)
                table->Columns[table->DisplayOrderToIndex[order_n]].DisplayOrder -= static_cast<
            ...
            for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
                table->DisplayOrderToIndex[table->Columns[column_n].DisplayOrder] = static_cast<
    ...
    if (table->IsResetDisplayOrderRequest)
    {
        for (int n = 0; n < table->ColumnsCount; n++)
            table->DisplayOrderToIndex[n] = table->Columns[n].DisplayOrder = static_cast<
                                  ...
                                  ImGuiTableColumnFlags flags_in)
{
    ...
    // Sorting
    if ((flags & ImGuiTableColumnFlags_NoSortAscending) && (flags &
    ...
    column->SortDirectionsAvailCount = column->SortDirectionsAvailMask = column->
        SortDirectionsAvailList = 0;
    if (table->Flags & ImGuiTableFlags_Sortable)
    {
        int count = 0, mask = 0, list = 0;
        if ((flags & ImGuiTableColumnFlags_PreferSortAscending) != 0 && (flags &
            ImGuiTableColumnFlags_NoSortAscending) == 0)
        {
            mask |= 1 << ImGuiSortDirection_Ascending;
            list |= ImGuiSortDirection_Ascending << (count << 1);
            count++;
        }
        if ((flags & ImGuiTableColumnFlags_PreferSortDescending) != 0 && (flags &
            ImGuiTableColumnFlags_NoSortDescending) == 0)
        {
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    // g.Style.ColumnsMinSpacing; // FIXME-TABLE
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        if (!(table->Flags & ImGuiTableFlags_Hideable) || (column->Flags &
        ...
        column->IsEnabled = column->IsUserEnabled && (column->Flags &
        ...
        if (prev_visible_column_idx != -1)
            table->Columns[prev_visible_column_idx].NextEnabledColumn = static_cast<
                ImGuiTableColumnIdx>(column_n);
        else
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        if (column->Flags & ImGuiTableColumnFlags_WidthFixed)
        {
            ...
            float width_auto = column->WidthAuto;
            if (table_sizing_policy == ImGuiTableFlags_SizingFixedSame && (column->AutoFitQueue !=
                0x00 || !column_is_resizable))
        ...
        else
        {
                ...
                column_is_resizable)
            {
                if (column->InitStretchWeightOrWidth > 0.0f)
                    ...
                    column->StretchWeight = (column->WidthAuto / stretch_sum_width_auto) *
                        count_stretch;
                else
            ...
            if (table->LeftMostStretchedColumn == -1 || table->Columns[table->
                LeftMostStretchedColumn].DisplayOrder > column->DisplayOrder)
                table->LeftMostStretchedColumn = static_cast<ImGuiTableColumnIdx>(column_n);
            if (table->RightMostStretchedColumn == -1 || table->Columns[table->
                RightMostStretchedColumn].DisplayOrder < column->DisplayOrder)
    ...
    const float width_spacings = (table->OuterPaddingX * 2.0f) + (table->CellSpacingX1 + table->
        CellSpacingX2) * (table->ColumnsEnabledCount - 1);
    const float width_removed = (table->HasScrollbarYPrev && !table->InnerWindow->ScrollbarY)
                                    ? g.Style.ScrollbarSize
                                    : 0.0f;
    // To synchronize decoration width of synched tables with mismatching scrollbar state (#5920)
    const float width_avail = ImMax(
        1.0f, (((table->Flags & ImGuiTableFlags_ScrollX) && table->InnerWidth == 0.0f)
                   ? table->InnerClipRect.GetWidth()
                   : work_rect.GetWidth()) - width_removed);
    const float width_avail_for_stretched_columns = width_avail - width_spacings -
        sum_width_requests;
    float width_remaining_for_stretched_columns = width_avail_for_stretched_columns;
    table->ColumnsGivenWidth = width_spacings + (table->CellPaddingX * 2.0f) * table->
        ColumnsEnabledCount;
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        if (!IM_BITARRAY_TESTBIT(table->EnabledMaskByIndex, column_n))
    ...
    if (width_remaining_for_stretched_columns >= 1.0f && !(table->Flags &
        ImGuiTableFlags_PreciseWidths))
        for (int order_n = table->ColumnsCount - 1; stretch_sum_weights > 0.0f &&
             width_remaining_for_stretched_columns >= 1.0f && order_n >= 0; order_n--)
        {
    ...
    table->HoveredColumnBody = table->HoveredColumnBorder = -1;
    const ImRect mouse_hit_rect(table->OuterRect.Min.x, table->OuterRect.Min.y,
    ...
    if (table->AngledHeadersHeight > 0.0f)
        if (g.IO.MousePos.y >= table->OuterRect.Min.y && g.IO.MousePos.y <= table->OuterRect.Min.y +
            table->AngledHeadersHeight)
            mouse_skewed_x += ImTrunc(
                (table->OuterRect.Min.y + table->AngledHeadersHeight - g.IO.MousePos.y) * table->
                AngledHeadersSlope);
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        column->NavLayerCurrent = static_cast<ImS8>(table->FreezeRowsCount > 0
                                                        ? ImGuiNavLayer_Menu
                                                        : ImGuiNavLayer_Main);
        // Use Count NOT request so Header line changes layer when frozen
        ...
        if (!IM_BITARRAY_TESTBIT(table->EnabledMaskByDisplayOrder, order_n))
        {
            ...
            column->MinX = column->MaxX = column->WorkMinX = column->ClipRect.Min.x = column->
                ClipRect.Max.x = offset_x;
        ...
        if (is_hovering_table && mouse_skewed_x >= column->ClipRect.Min.x && mouse_skewed_x < column
            ->ClipRect.Max.x)
        ...
        column->IsRequestOutput = is_visible || column->AutoFitQueue != 0 || column->
        ...
        offset_x += column->WidthGiven + table->CellSpacingX1 + table->CellSpacingX2 + table->
            CellPaddingX * 2.0f;
        visible_n++;
    }
    ...
    if (table->IsContextPopupOpen && table->ContextPopupColumn != -1 && table->InstanceInteracted ==
        table->InstanceCurrent)
        table->HighlightColumnHeader = table->ContextPopupColumn;
    else if ((table->Flags & ImGuiTableFlags_HighlightHoveredColumn) && table->HoveredColumnBody !=
        -1 && table->HoveredColumnBody != table->ColumnsCount && table->HoveredColumnBorder == -1)
    ...
    if (table->FreezeColumnsRequest > 0)
        table->InnerWindow->DecoInnerSizeX1 = table->Columns[table->DisplayOrderToIndex[table->
            FreezeColumnsRequest - 1]].MaxX - table->OuterRect.Min.x;
...
void ImGui::TableUpdateBorders(ImGuiTable* table)
{
    ...
    const float hit_y1 = (table->FreezeRowsCount >= 1
...
void ImGui::EndTable()
{
    ...
    if (flags & ImGuiTableFlags_ContextMenuInBody)
        if (table->HoveredColumnBody != -1 && !IsAnyItemHovered() && IsMouseReleased(
    ...
    if (inner_window != outer_window)
        ...
        table->OuterRect.Max.y = table->InnerRect.Max.y = ImMax(
            table->OuterRect.Max.y, inner_content_max_y); // Patch OuterRect/InnerRect height
    ...
    {
        const float outer_padding_for_border = (table->Flags & ImGuiTableFlags_BordersOuterV)
                                                   ? TABLE_BORDER_SIZE
                                                   : 0.0f;
        float max_pos_x = table->InnerWindow->DC.CursorMaxPos.x;
        if (table->RightMostEnabledColumn != -1)
            max_pos_x = ImMax(max_pos_x,
                              table->Columns[table->RightMostEnabledColumn].WorkMaxX + table->
                              CellPaddingX + table->OuterPaddingX - outer_padding_for_border);
        if (table->ResizedColumn != -1)
            max_pos_x = ImMax(max_pos_x, table->ResizeLockMinContentsX2);
        table->InnerWindow->DC.CursorMaxPos.x = max_pos_x + table->TempData->
            AngledheadersExtraWidth;
    }
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
        if (IM_BITARRAY_TESTBIT(table->EnabledMaskByIndex, column_n))
        {
            ...
            if ((column->Flags & ImGuiTableColumnFlags_WidthStretch) && (column->Flags &
                ImGuiTableColumnFlags_NoResize) != 0)
                auto_fit_width_for_stretched_min = ImMax(auto_fit_width_for_stretched_min,
                                                         column_width_request / (column->
                                                             StretchWeight / table->
                                                             ColumnsStretchSumWeights));
        }
    const float width_spacings = (table->OuterPaddingX * 2.0f) + (table->CellSpacingX1 + table->
        CellSpacingX2) * (table->ColumnsEnabledCount - 1);
    table->ColumnsAutoFitWidth = width_spacings + (table->CellPaddingX * 2.0f) * table->
        ColumnsEnabledCount + auto_fit_width_for_fixed + ImMax(
            auto_fit_width_for_stretched, auto_fit_width_for_stretched_min);
    ...
    else if (table->LastResizedColumn != -1 && table->ResizedColumn == -1 && inner_window->
        ScrollbarX && table->InstanceInteracted == table->InstanceCurrent)
    {
        // When releasing a column being resized, scroll to keep the resulting column in sight
        const float neighbor_width_to_keep_visible = table->MinColumnWidth + table->CellPaddingX *
            2.0f;
    ...
    if (table->ResizedColumn != -1 && table->InstanceCurrent == table->InstanceInteracted)
    {
        ...
        const float new_width = ImTrunc(
            new_x2 - column->MinX - table->CellSpacingX1 - table->CellPaddingX * 2.0f);
        table->ResizedColumnNextWidth = new_width;
    }

    table->IsActiveIdInTable = (g.ActiveIdIsAlive != 0 && table->IsActiveIdAliveBeforeTable ==
        false);
    ...
    if (temp_data->UserOuterSize.y <= 0.0f)
    {
        ...
        outer_window->DC.IdealMaxPos.y = ImMax(outer_window->DC.IdealMaxPos.y,
                                               inner_content_max_y + decoration_size - temp_data->
                                               UserOuterSize.y);
                             ...
                             float init_width_or_weight, ImGuiID user_id)
{
    ...
    if ((flags & ImGuiTableColumnFlags_WidthMask_) == 0 && init_width_or_weight > 0.0f)
        if ((table->Flags & ImGuiTableFlags_SizingMask_) == ImGuiTableFlags_SizingFixedFit || (table
    ...
    if (table->IsInitializing)
    {
        ...
        if ((flags & ImGuiTableColumnFlags_DefaultHide) && (table->SettingsLoadedFlags &
            ImGuiTableFlags_Hideable) == 0)
            column->IsUserEnabled = column->IsUserEnabledNextFrame = false;
        if (flags & ImGuiTableColumnFlags_DefaultSort && (table->SettingsLoadedFlags &
            ImGuiTableFlags_Sortable) == 0)
        {
            column->SortOrder = 0;
            ...
            column->SortDirection = (column->Flags & ImGuiTableColumnFlags_PreferSortDescending)
                                        ? static_cast<ImS8>(ImGuiSortDirection_Descending)
                                        : static_cast<ImU8>(ImGuiSortDirection_Ascending);
        }
    }
...
void ImGui::TableSetupScrollFreeze(int columns, int rows)
{
    ...
    table->FreezeColumnsRequest = (table->Flags & ImGuiTableFlags_ScrollX)
                                      ? static_cast<ImGuiTableColumnIdx>(ImMin(
    ...
    table->FreezeRowsRequest = (table->Flags & ImGuiTableFlags_ScrollY)
                                   ? static_cast<ImGuiTableColumnIdx>(rows)
                                   : 0;
    ...
    case ImGuiTableBgTarget_CellBg:
        {
            ...
            if (!IM_BITARRAY_TESTBIT(table->VisibleMaskByIndex, column_n))
                return;
            if (table->RowCellDataCurrent < 0 || table->RowCellData[table->RowCellDataCurrent].
                Column != column_n)
...
void ImGui::TableBeginRow(ImGuiTable* table)
{
    ...
    window->DC.CursorPosPrevLine = ImVec2(window->DC.CursorPos.x,
                                          window->DC.CursorPos.y + table->RowCellPaddingY);
    // This allows users to call SameLine() to share LineSize between columns.
    window->DC.PrevLineSize = window->DC.CurrLineSize = ImVec2(0.0f, 0.0f);
    // This allows users to call SameLine() to share LineSize between columns, and to call it from first column too.
...
void ImGui::TableEndRow(ImGuiTable* table)
{
    ...
    ImGuiTableInstanceData* table_instance = TableGetInstanceData(table, table->InstanceCurrent);
    if ((table->RowFlags & ImGuiTableRowFlags_Headers) && (table->CurrentRow == 0 || (table->
        LastRowFlags & ImGuiTableRowFlags_Headers)))
        table_instance->LastTopHeadersRowHeight += bg_y2 - bg_y1;

    const bool is_visible = (bg_y2 >= table->InnerClipRect.Min.y && bg_y1 <= table->InnerClipRect.
        Max.y);
    if (is_visible)
    {
        ...
        if (top_border_col && bg_y1 >= table->BgClipRect.Min.y && bg_y1 < table->BgClipRect.Max.y)
            window->DrawList->AddLine(ImVec2(table->BorderX1, bg_y1),
                                      ImVec2(table->BorderX2, bg_y1), top_border_col, border_size);

        // Draw bottom border at the row unfreezing mark (always strong)
        if (draw_strong_bottom_border && bg_y2 >= table->BgClipRect.Min.y && bg_y2 < table->
            BgClipRect.Max.y)
    ...
    if (unfreeze_rows_actual)
    {
        ...
        table->BgClipRect.Min.y = table->Bg2ClipRectForDrawCmd.Min.y = ImMin(
            y0, window->InnerClipRect.Max.y);
        table->BgClipRect.Max.y = table->Bg2ClipRectForDrawCmd.Max.y = window->InnerClipRect.Max.y;
        table->Bg2DrawChannelCurrent = table->Bg2DrawChannelUnfrozen;
        IM_ASSERT(table->Bg2ClipRectForDrawCmd.Min.y <= table->Bg2ClipRectForDrawCmd.Max.y);

        float row_height = table->RowPosY2 - table->RowPosY1;
        table->RowPosY2 = window->DC.CursorPos.y = table->WorkRect.Min.y + table->RowPosY2 - table->
            OuterRect.Min.y;
...
void ImGui::TableBeginCell(ImGuiTable* table, int column_n)
{
    ...
    // PrevLine.y is preserved. This allows users to call SameLine() to share LineSize between columns.
...
float ImGui::TableGetMaxColumnWidth(const ImGuiTable* table, int column_n)
{
    ...
    const float min_column_distance = table->MinColumnWidth + table->CellPaddingX * 2.0f + table->
        CellSpacingX1 + table->CellSpacingX2;
    if (table->Flags & ImGuiTableFlags_ScrollX)
    {
        ...
        {
            max_width = (table->InnerClipRect.Max.x - (table->FreezeColumnsRequest - column->
                DisplayOrder) * min_column_distance) - column->MinX;
            max_width = max_width - table->OuterPaddingX - table->CellPaddingX - table->
                CellSpacingX2;
        }
    }
    else if ((table->Flags & ImGuiTableFlags_NoKeepColumnsVisible) == 0)
    {
        ...
        max_width = table->WorkRect.Max.x - (table->ColumnsEnabledCount - column->
            IndexWithinEnabledSet - 1) * min_column_distance - column->MinX;
...
{
    const float content_width_body = ImMax(column->ContentMaxXFrozen, column->ContentMaxXUnfrozen) -
        column->WorkMinX;
    ...
    if ((column->Flags & ImGuiTableColumnFlags_WidthFixed) && column->InitStretchWeightOrWidth >
        0.0f)
        if (!(table->Flags & ImGuiTableFlags_Resizable) || (column->Flags &
...
void ImGui::TableSetColumnWidth(int column_n, float width)
{
    ...
    if (column_0->Flags & ImGuiTableColumnFlags_WidthFixed)
        if (!column_1 || table->LeftMostStretchedColumn == -1 || table->Columns[table->
            LeftMostStretchedColumn].DisplayOrder >= column_0->DisplayOrder)
        {
    ...
    float column_1_width = ImMax(column_1->WidthRequest - (column_0_width - column_0->WidthRequest),
                                 min_width);
...
void ImGui::TableSetupDrawChannels(ImGuiTable* table)
{
    ...
    const int channels_for_dummy = (table->ColumnsEnabledCount < table->ColumnsCount || (memcmp(
    ...
    table->DummyDrawChannel = static_cast<ImGuiTableDrawChannelIdx>((channels_for_dummy > 0)
        ? channels_total - 1
        : -1);
    table->Bg2DrawChannelCurrent = TABLE_DRAW_CHANNEL_BG2_FROZEN;
    table->Bg2DrawChannelUnfrozen = static_cast<ImGuiTableDrawChannelIdx>((table->FreezeRowsCount >
        0)
        ? 2 + channels_for_row
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        if (column->IsVisibleX && column->IsVisibleY)
        {
            ...
            column->DrawChannelUnfrozen = static_cast<ImGuiTableDrawChannelIdx>(draw_channel_current
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    const int size_for_masks_bitarrays_one = static_cast<int>(ImBitArrayGetStorageSizeInBytes(
        max_draw_channels));
    ...
    auto remaining_mask = static_cast<ImBitArrayPtr>((void*)(g.TempBuffer.Data + (
             ...
             ++)
        {
            ...
            if (!(column->Flags & ImGuiTableColumnFlags_NoClip))
            {
                ...
                if (!has_freeze_v)
                    ...
                    content_max_x =
                        ImMax(column->ContentMaxXFrozen, column->ContentMaxXHeadersUsed);
    ...
    if (merge_group_mask != 0)
    {
        ...
        IM_ASSERT(
            has_freeze_v == false || table->Bg2DrawChannelUnfrozen !=
            TABLE_DRAW_CHANNEL_BG2_FROZEN);
        int remaining_count = splitter->_Count - (has_freeze_v
                                                      ? LEADING_DRAW_CHANNELS + 1
        ...
        for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            if (int merge_channels_count = merge_groups[merge_group_n].ChannelsCount)
            {
                ...
                for (int n = 0; n < splitter->_Count && merge_channels_count != 0; n++)
                {
                    ...
                    IM_ASSERT(
                        channel->_CmdBuffer.Size == 1 && merge_clip_rect.Contains(ImRect(channel->
                            _CmdBuffer[0].ClipRect)));
...
static ImU32 TableGetColumnBorderCol(ImGuiTable* table, int order_n, int column_n)
{
    ...
    const bool is_resized = (table->ResizedColumn == column_n) && (table->InstanceInteracted ==
        table->InstanceCurrent);
    const bool is_frozen_separator = (table->FreezeColumnsCount == order_n + 1);
    if (is_resized || is_hovered)
        return ImGui::GetColorU32(is_resized
...
void ImGui::TableDrawBorders(ImGuiTable* table)
{
    ...
    const float draw_y2_head = table->IsUsingHeaders
                                   ? ImMin(table->InnerRect.Max.y,
                                           (table->FreezeRowsCount >= 1
                                                ? table->InnerRect.Min.y
                                                : table->WorkRect.Min.y) + table_instance->
    ...
    if (table->Flags & ImGuiTableFlags_BordersInnerV)
    {
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            float draw_y2 = (is_hovered || is_resized || is_frozen_separator || (table->Flags & (
    ...
    if (table->Flags & ImGuiTableFlags_BordersOuter)
    {
        ...
        {
            inner_drawlist->AddRect(outer_border.Min, outer_border.Max + ImVec2(1, 1), outer_col,
                                    0.0f, 0, border_size);
        }
...
{
    if (column->SortOrder == -1 || (column->SortDirectionsAvailMask & (1 << column->SortDirection))
        != 0)
        return;
...
IM_STATIC_ASSERT(
    ImGuiSortDirection_None == 0 && ImGuiSortDirection_Ascending == 1 &&
...
void ImGui::TableSortSpecsSanitize(ImGuiTable* table)
{
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        IM_ASSERT(sort_order_count < static_cast<int>(sizeof(sort_order_mask)) * 8);
    }

    const bool need_fix_linearize = (static_cast<ImU64>(1) << sort_order_count) != (sort_order_mask
        + 1);
    const bool need_fix_single_sort_order = (sort_order_count > 1) && !(table->Flags &
    ...
    if (need_fix_linearize || need_fix_single_sort_order)
    {
        ...
        for (int sort_n = 0; sort_n < sort_order_count; sort_n++)
        {
            ...
            for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
                if ((fixed_mask & (static_cast<ImU64>(1) << static_cast<ImU64>(column_n))) == 0 &&
            ...
            table->Columns[column_with_smallest_sort_order].SortOrder = static_cast<
    ...
    if (sort_order_count == 0 && !(table->Flags & ImGuiTableFlags_SortTristate))
        for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
        {
            ...
            if (column->IsEnabled && !(column->Flags & ImGuiTableColumnFlags_NoSort))
            {
                sort_order_count = 1;
                column->SortOrder = 0;
                column->SortDirection = static_cast<ImU8>(TableGetColumnAvailSortDirection(
                    column, 0));
                break;
            }
        }
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
        if (IM_BITARRAY_TESTBIT(table->EnabledMaskByIndex, column_n))
            if (table->Columns[column_n].Flags & ImGuiTableColumnFlags_AngledHeader)
                width = ImMax(
                    width, CalcTextSize(TableGetColumnName(table, column_n), nullptr, true).x);
...
void ImGui::TableHeader(const char* label)
{
    ...
    if ((table->Flags & ImGuiTableFlags_Sortable) && !(column->Flags &
    ...
    column->ContentMaxXHeadersUsed = ImMax(column->ContentMaxXHeadersUsed,
                                           sort_arrow
                                               ? cell_r.Max.x
                                               : ImMin(max_pos_x, cell_r.Max.x));
    ...
    // Don't declare unclipped width, it'll be fed ContentMaxPosHeadersIdeal
    if (!ItemAdd(bb, id))
        return;
    ...
    if (held && (table->Flags & ImGuiTableFlags_Reorderable) && IsMouseDragging(0) && !g.
        DragDropActive)
    {
...
void ImGui::TableAngledHeadersRowEx(float angle, float max_label_width)
{
    ...
    const auto unit_right = ImVec2(cos_a, sin_a);
    ...
    draw_list->AddRectFilled(table->BgClipRect.Min, table->BgClipRect.Max,
                             GetColorU32(ImGuiCol_TableHeaderBg, 0.25f));
    // FIXME-STYLE: Change row background with an arbitrary color.
    ...
    if (highlight_column_n == -1 && table->HoveredColumnBody != -1)
        if (table_instance->HoveredRowLast == 0 && table->HoveredColumnBorder == -1 && (g.ActiveId
            == 0 || g.ActiveId == row_id || (table->IsActiveIdInTable || g.DragDropActive)))
    ...
    for (int pass = 0; pass < 2; pass++)
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            if (pass == 0)
            {
                ...
                ImRect label_r(window->ClipRect.Min,
                               window->ClipRect.Min + ImVec2(
                ...
                auto label_off = ImVec2(
                    flip_label
                        ? ImMax(0.0f, max_label_width - label_size.x - table->CellPaddingX)
                        : table->CellPaddingX, table->RowCellPaddingY);
                int vtx_idx_begin = draw_list->_VtxCurrentIdx;
                RenderTextEllipsis(draw_list, label_r.Min + label_off, label_r.Max, label_r.Max.x,
                                   label_r.Max.x, label_name, nullptr, &label_size);
                ...
                ImVec2 pivot_out = ImVec2(column->WorkMinX, row_r.Max.y) + (flip_label
                    ? (unit_right * clip_width)
                    : ImVec2(header_height, 0.0f));
                                        ...
                                        ImGuiTableFlags flags_for_section_to_display)
{
    ...
    const int column_n = (table->ContextPopupColumn >= 0 && table->ContextPopupColumn < table->
                             ColumnsCount)
    ...
    if (flags_for_section_to_display & ImGuiTableFlags_Resizable)
    {
        if (column != nullptr)
        {
            const bool can_resize = !(column->Flags & ImGuiTableColumnFlags_NoResize) && column->
                IsEnabled;
            if (MenuItem(LocalizeGetMsg(ImGuiLocKey_TableSizeOne), nullptr, false, can_resize))
                // "###SizeOne"
                TableSetColumnWidthAutoSingle(table, column_n);
        }

        const char* size_all_desc;
        if (table->ColumnsEnabledFixedCount == table->ColumnsEnabledCount && (table->Flags &
    ...
    {
        if (MenuItem(LocalizeGetMsg(ImGuiLocKey_TableResetOrder), nullptr, false,
    ...
    if (flags_for_section_to_display & ImGuiTableFlags_Hideable)
    {
        ...
        for (int other_column_n = 0; other_column_n < table->ColumnsCount; other_column_n++)
        {
            ...
            bool menu_item_active = (other_column->Flags & ImGuiTableColumnFlags_NoHide)
                                        ? false
                                        : true;
...
{
    return sizeof(ImGuiTableSettings) + static_cast<size_t>(columns_count) * sizeof(
...
ImGuiTableSettings* ImGui::TableSettingsFindByID(ImGuiID id)
{
    ...
    ImGuiContext& g = *GImGui;
    for (ImGuiTableSettings* settings = g.SettingsTables.begin(); settings != nullptr; settings = g.
         SettingsTables.next_chunk(settings))
        if (settings->ID == id)
            return settings;
    return nullptr;
}
...
void ImGui::TableSaveSettings(ImGuiTable* table)
{
    ...
    IM_ASSERT(settings->ID == table->ID);
    IM_ASSERT(
        settings->ColumnsCount == table->ColumnsCount && settings->ColumnsCountMax >= settings->
        ColumnsCount);
                                          ...
                                          const char* line)
{
    ...
    auto settings = static_cast<ImGuiTableSettings*>(entry);
    float f = 0.0f;
    ...
    if (sscanf(line, "Column %d%n", &column_n, &r) == 1)
    {
        ...
        {
            line = ImStrSkipBlank(line + r);
            column->UserID = static_cast<ImGuiID>(n);
        }
        if (sscanf(line, "Width=%d%n", &n, &r) == 1)
        {
            line = ImStrSkipBlank(line + r);
            column->WidthOrWeight = static_cast<float>(n);
            column->IsStretch = 0;
            settings->SaveFlags |= ImGuiTableFlags_Resizable;
        }
        ...
        {
            line = ImStrSkipBlank(line + r);
            column->IsEnabled = static_cast<ImU8>(n);
            settings->SaveFlags |= ImGuiTableFlags_Hideable;
        }
        if (sscanf(line, "Order=%d%n", &n, &r) == 1)
        {
            line = ImStrSkipBlank(line + r);
            column->DisplayOrder = static_cast<ImGuiTableColumnIdx>(n);
            settings->SaveFlags |= ImGuiTableFlags_Reorderable;
        }
        if (sscanf(line, "Sort=%d%c%n", &n, &c, &r) == 2)
        {
            line = ImStrSkipBlank(line + r);
            column->SortOrder = static_cast<ImGuiTableColumnIdx>(n);
            column->SortDirection = (c == '^')
                                          ...
                                          ImGuiTextBuffer* buf)
{
         ...
         SettingsTables.next_chunk(settings))
    {
        ...
        for (int column_n = 0; column_n < settings->ColumnsCount; column_n++, column++)
        {
            ...
            if (save_size && !column->IsStretch)
            {
                buf->appendf(" Width=%d", static_cast<int>(column->WidthOrWeight));
            }
...
void ImGui::TableGcCompactTransientBuffers(ImGuiTable* table)
{
    ...
    table->IsSortSpecsDirty = true;
    // FIXME: In theory shouldn't have to leak into user performing a sort on resume.
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    for (int n = 0; n < table->ColumnsCount; n++)
    {
        ...
        ImFormatString(buf, IM_ARRAYSIZE(buf),
                       ...
                       column->IsRequestOutput, column->IsSkipItems, column->DrawChannelFrozen,
                       column->DrawChannelUnfrozen,
...
{
    if (!TreeNode((void*)static_cast<intptr_t>(settings->ID), "Settings 0x%08X (%d columns)",
    ...
    for (int n = 0; n < settings->ColumnsCount; n++)
    {
        ...
        ImGuiSortDirection sort_dir = (column_settings->SortOrder != -1)
                                          ? static_cast<ImGuiSortDirection>(column_settings->
                                              SortDirection)
...
static float GetDraggedColumnOffset(ImGuiOldColumns* columns, int column_index)
{
    ...
    IM_ASSERT(g.ActiveId == columns->ID + static_cast<ImGuiID>(column_index));

    float x = g.IO.MousePos.x - g.ActiveIdClickOffset.x + COLUMNS_HIT_RECT_HALF_WIDTH - window->Pos.
        x;
                              ...
                              bool before_resize = false)
{
    ...
    float offset_norm;
    if (before_resize)
        offset_norm = columns->Columns[column_index + 1].OffsetNormBeforeResize - columns->Columns[
            column_index].OffsetNormBeforeResize;
    else
        offset_norm = columns->Columns[column_index + 1].OffsetNorm - columns->Columns[column_index]
            .OffsetNorm;
...
void ImGui::SetColumnOffset(int column_index, float offset)
{
    ...
    columns->Columns[column_index].OffsetNorm = GetColumnNormFromOffset(
        columns, offset - columns->OffMinX);
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiOldColumnFlags flags)
{
    ...
    const float half_clip_extend_x = ImTrunc(ImMax(window->WindowPadding.x * 0.5f,
                                                   window->WindowBorderSize));
    const float max_1 = window->WorkRect.Max.x + column_padding - ImMax(
        column_padding - window->WindowPadding.x, 0.0f);
    const float max_2 = window->WorkRect.Max.x + half_clip_extend_x;
    columns->OffMinX = window->DC.Indent.x - column_padding + ImMax(
        column_padding - window->WindowPadding.x, 0.0f);
