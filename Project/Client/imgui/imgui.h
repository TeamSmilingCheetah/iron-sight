...
using ImGuiCol = int; // -> enum ImGuiCol_             // Enum: A color identifier for styling
using ImGuiCond = int;
// -> enum ImGuiCond_            // Enum: A condition for many Set*() functions
using ImGuiDataType = int; // -> enum ImGuiDataType_        // Enum: A primary data type
using ImGuiDir = int; // -> enum ImGuiDir_             // Enum: A cardinal direction
using ImGuiMouseButton = int;
// -> enum ImGuiMouseButton_     // Enum: A mouse button identifier (0=left, 1=right, 2=middle)
using ImGuiMouseCursor = int; // -> enum ImGuiMouseCursor_     // Enum: A mouse cursor shape
...
//   In Visual Studio IDE: CTRL+comma ("Edit.GoToAll") can follow symbols in comments, whereas CTRL+F12 ("Edit.GoToImplementation") cannot.
//   With Visual Assist installed: ALT+G ("VAssistX.GoToImplementation") can also follow symbols in comments.
using ImDrawFlags = int; // -> enum ImDrawFlags_          // Flags: for ImDrawList functions
using ImDrawListFlags = int; // -> enum ImDrawListFlags_      // Flags: for ImDrawList instance
using ImFontAtlasFlags = int; // -> enum ImFontAtlasFlags_     // Flags: for ImFontAtlas build
using ImGuiBackendFlags = int; // -> enum ImGuiBackendFlags_    // Flags: for io.BackendFlags
using ImGuiButtonFlags = int; // -> enum ImGuiButtonFlags_     // Flags: for InvisibleButton()
using ImGuiChildFlags = int; // -> enum ImGuiChildFlags_      // Flags: for BeginChild()
using ImGuiColorEditFlags = int;
// -> enum ImGuiColorEditFlags_  // Flags: for ColorEdit4(), ColorPicker4() etc.
using ImGuiConfigFlags = int; // -> enum ImGuiConfigFlags_     // Flags: for io.ConfigFlags
using ImGuiComboFlags = int; // -> enum ImGuiComboFlags_      // Flags: for BeginCombo()
using ImGuiDockNodeFlags = int; // -> enum ImGuiDockNodeFlags_   // Flags: for DockSpace()
using ImGuiDragDropFlags = int;
// -> enum ImGuiDragDropFlags_   // Flags: for BeginDragDropSource(), AcceptDragDropPayload()
using ImGuiFocusedFlags = int; // -> enum ImGuiFocusedFlags_    // Flags: for IsWindowFocused()
...
using ImGuiSelectableFlags = int; // -> enum ImGuiSelectableFlags_ // Flags: for Selectable()
using ImGuiSliderFlags = int;
// -> enum ImGuiSliderFlags_     // Flags: for DragFloat(), DragInt(), SliderFloat(), SliderInt() etc.
using ImGuiTabBarFlags = int; // -> enum ImGuiTabBarFlags_     // Flags: for BeginTabBar()
using ImGuiTabItemFlags = int; // -> enum ImGuiTabItemFlags_    // Flags: for BeginTabItem()
using ImGuiTableFlags = int; // -> enum ImGuiTableFlags_      // Flags: For BeginTable()
using ImGuiTableColumnFlags = int; // -> enum ImGuiTableColumnFlags_// Flags: For TableSetupColumn()
using ImGuiTableRowFlags = int; // -> enum ImGuiTableRowFlags_   // Flags: For TableNextRow()
using ImGuiTreeNodeFlags = int;
// -> enum ImGuiTreeNodeFlags_   // Flags: for TreeNode(), TreeNodeEx(), CollapsingHeader()
using ImGuiViewportFlags = int; // -> enum ImGuiViewportFlags_   // Flags: for ImGuiViewport
using ImGuiWindowFlags = int; // -> enum ImGuiWindowFlags_     // Flags: for Begin(), BeginChild()
...
struct ImVec2
{
    ...
    float& operator[](size_t idx)
    {
        ...
        return static_cast<float*>((void*)(char*)this)[idx];
    } // We very rarely use this [] operator, so the assert overhead is fine.
    float operator[](size_t idx) const
    {
        IM_ASSERT(idx == 0 || idx == 1);
        return static_cast<const float*>((const void*)(const char*)this)[idx];
    }
...
namespace ImGui
{
    ...
    IMGUI_API void ShowIDStackToolWindow(bool* p_open = nullptr);
    // create Stack Tool window. hover items with mouse to query information about the source of their unique ID.
    IMGUI_API void ShowAboutWindow(bool* p_open = nullptr);
    // create About window. display Dear ImGui version, credits and build/system information.
    IMGUI_API void ShowStyleEditor(ImGuiStyle* ref = nullptr);
    ...
    IMGUI_API bool DragScalar(const char* label, ImGuiDataType data_type, void* p_data,
                              float v_speed = 1.0f, const void* p_min = nullptr,
    ...
    IMGUI_API void PlotLines(const char* label, float (*values_getter)(void* data, int idx),
                             ...
                             const char* overlay_text = nullptr, float scale_min = FLT_MAX,
    ...
    IMGUI_API bool MenuItem(const char* label, const char* shortcut = nullptr,
                            bool selected = false,
    ...
    IMGUI_API bool BeginPopupModal(const char* name, bool* p_open = nullptr,
    ...
    IMGUI_API void
    OpenPopupOnItemClick(const char* str_id = nullptr, ImGuiPopupFlags popup_flags = 1);
    ...
    IMGUI_API bool
    BeginPopupContextItem(const char* str_id = nullptr, ImGuiPopupFlags popup_flags = 1);
    // open+begin popup when clicked on last item. Use str_id==NULL to associate the popup to previous item. If you want to use that on a non-interactive item such as Text() you need to pass in an explicit ID here. read comments in .cpp!
    IMGUI_API bool BeginPopupContextWindow(const char* str_id = nullptr,
                                           ImGuiPopupFlags popup_flags = 1);
    // open+begin popup when clicked on current window.
    IMGUI_API bool BeginPopupContextVoid(const char* str_id = nullptr,
                                         ImGuiPopupFlags popup_flags = 1);
    ...
    IMGUI_API bool BeginTabItem(const char* label, bool* p_open = nullptr,
    ...
    IMGUI_API ImGuiID DockSpaceOverViewport(const ImGuiViewport* viewport = nullptr,
                                            ImGuiDockNodeFlags flags = 0,
                                            const ImGuiWindowClass* window_class = nullptr);
    ...
    // Text Utilities
    IMGUI_API ImVec2 CalcTextSize(const char* text, const char* text_end = nullptr,
    ...
    IMGUI_API void
    RenderPlatformWindowsDefault(void* platform_render_arg = nullptr,
                                 void* renderer_render_arg = nullptr);
...
enum ImGuiHoveredFlags_
{
    ...
    ImGuiHoveredFlags_RootAndChildWindows = ImGuiHoveredFlags_RootWindow |
...
enum ImGuiKey : int
{
    ...
    ImGuiKey_I, ImGuiKey_J,
    ImGuiKey_K, ImGuiKey_L, ImGuiKey_M, ImGuiKey_N, ImGuiKey_O, ImGuiKey_P, ImGuiKey_Q, ImGuiKey_R,
    ImGuiKey_S, ImGuiKey_T,
    ...
    ImGuiKey_MouseLeft, ImGuiKey_MouseRight, ImGuiKey_MouseMiddle, ImGuiKey_MouseX1,
    ...
    ImGuiKey_ModAlt = ImGuiMod_Alt, ImGuiKey_ModSuper = ImGuiMod_Super, // Renamed in 1.89
...
enum ImGuiNavInput
{
    ...
    ImGuiNavInput_LStickDown, ImGuiNavInput_FocusPrev, ImGuiNavInput_FocusNext,
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags_PickerMask_ = ImGuiColorEditFlags_PickerHueWheel |
...
struct ImVector
{
    ...
    ImVector<T>& operator=(const ImVector<T>& src)
    {
        clear();
        resize(src.Size);
        if (src.Data) memcpy(Data, src.Data, static_cast<size_t>(Size) * sizeof(T));
        return *this;
    }
    ...
    int size_in_bytes() const { return Size * static_cast<int>(sizeof(T)); }
    int max_size() const { return 0x7FFFFFFF / static_cast<int>(sizeof(T)); }
    ...
    void reserve(int new_capacity)
    {
        ...
        T* new_data = static_cast<T*>(IM_ALLOC((size_t)new_capacity * sizeof(T)));
        if (Data)
        {
            memcpy(new_data, Data, static_cast<size_t>(Size) * sizeof(T));
            IM_FREE(Data);
        }
        Data = new_data;
    ...
    void reserve_discard(int new_capacity)
    {
        ...
        if (Data)
            IM_FREE(Data);
        Data = static_cast<T*>(IM_ALLOC((size_t)new_capacity * sizeof(T)));
        Capacity = new_capacity;
    }
    ...
    T* erase(const T* it)
    {
        ...
        memmove(Data + off, Data + off + 1,
                (static_cast<size_t>(Size) - static_cast<size_t>(off) - 1) * sizeof(T));
        Size--;
        return Data + off;
    }
    ...
    T* erase(const T* it, const T* it_last)
    {
        ...
        memmove(Data + off, Data + off + count,
                (static_cast<size_t>(Size) - static_cast<size_t>(off) - static_cast<size_t>(count))
                * sizeof(T));
    ...
    T* insert(const T* it, const T& v)
    {
        ...
        const ptrdiff_t off = it - Data;
        if (Size == Capacity) reserve(_grow_capacity(Size + 1));
        if (off < Size) memmove(Data + off + 1, Data + off,
                                (static_cast<size_t>(Size) - static_cast<size_t>(off)) * sizeof(T));
...
struct ImGuiIO
{
    ...
    // = NULL   // [Obsoleted in 1.87] Set ImGuiViewport::PlatformHandleRaw instead. Set this to your HWND to get automatic IME cursor positioning.
#else
    void*       _UnusedPadding;
#endif
...
struct ImGuiWindowClass
{
    ...
    ImGuiWindowClass()
    {
        ...
        ParentViewportId = static_cast<ImGuiID>(-1);
...
struct ImGuiListClipper
{
    ...
    void IncludeItemByIndex(int item_index) { IncludeItemsByIndex(item_index, item_index + 1); }
...
struct ImColor
{
    ...
    constexpr ImColor(int r, int g, int b, int a = 255) : Value(
        static_cast<float>(r) * (1.0f / 255.0f), static_cast<float>(g) * (1.0f / 255.0f),
        static_cast<float>(b) * (1.0f / 255.0f), static_cast<float>(a) * (1.0f / 255.0f))
    {
    }

    constexpr ImColor(ImU32 rgba) : Value(
        static_cast<float>((rgba >> IM_COL32_R_SHIFT) & 0xFF) * (1.0f / 255.0f),
        static_cast<float>((rgba >> IM_COL32_G_SHIFT) & 0xFF) * (1.0f / 255.0f),
        static_cast<float>((rgba >> IM_COL32_B_SHIFT) & 0xFF) * (1.0f / 255.0f),
        static_cast<float>((rgba >> IM_COL32_A_SHIFT) & 0xFF) * (1.0f / 255.0f))
    {
    }
...
struct ImDrawListSplitter
{
    ...
    ImDrawListSplitter() { memset(this, 0, sizeof(*this)); }
...
struct ImDrawList
{
    ...
    IMGUI_API void AddRectFilled(const ImVec2& p_min, const ImVec2& p_max, ImU32 col,
                                 float rounding = 0.0f,
                                 ImDrawFlags flags = 0);
    // a: upper-left, b: lower-right (== upper-left + size)
    ...
    IMGUI_API void AddEllipse(const ImVec2& center, float radius_x, float radius_y, ImU32 col,
                              float rot = 0.0f, int num_segments = 0, float thickness = 1.0f);
    IMGUI_API void AddEllipseFilled(const ImVec2& center, float radius_x, float radius_y, ImU32 col,
    ...
    {
        if (_Path.Size == 0 || memcmp(&_Path.Data[_Path.Size - 1], &pos, 8) != 0) _Path.
            push_back(pos);
    }
    ...
    IMGUI_API void PathEllipticalArcTo(const ImVec2& center, float radius_x, float radius_y,
...
struct ImFontGlyphRangesBuilder
{
    ...
    void Clear()
    {
        ...
        UsedChars.resize(size_in_bytes / static_cast<int>(sizeof(ImU32)));
        memset(UsedChars.Data, 0, static_cast<size_t>(size_in_bytes));
    }

    bool GetBit(size_t n) const
    {
        int off = static_cast<int>(n >> 5);
        ImU32 mask = 1u << (n & 31);
    ...
    IMGUI_API void AddText(const char* text, const char* text_end = nullptr);
...
struct ImFont
{
    ...
    {
        return (static_cast<int>(c) < IndexAdvanceX.Size)
                   ? IndexAdvanceX[static_cast<int>(c)]
                   : FallbackAdvanceX;
    }
    ...
    IMGUI_API ImVec2 CalcTextSizeA(float size, float max_width, float wrap_width,
                                   const char* text_begin, const char* text_end = nullptr,
                                   const char** remaining = nullptr) const; // utf8
...
namespace ImGui
{
    ...
    static inline void ShowStackToolWindow(bool* p_open = nullptr)
    {
...
using ImGuiModFlags = ImGuiKeyChord;
