...
namespace DirectX::SimpleMath
{
    ...
    struct Rectangle
    {
        ...
        XM_CONSTEXPR Rectangle(long ix, long iy, long iw, long ih) : x(ix), y(iy), width(iw),
            height(ih)
        {
        }
        ...
        {
            return (x == rct.left) && (y == rct.top) && (width == (rct.right - rct.left)) && (height
                == (rct.bottom - rct.top));
        }
        ...
        {
            return (x <= r.x) && ((r.x + r.width) <= (x + width)) && (y <= r.y) && ((r.y + r.height)
                <= (y + height));
        }

        bool Contains(const RECT& rct) const
        {
            return (x <= rct.left) && (rct.right <= (x + width)) && (y <= rct.top) && (rct.bottom <=
                (y + height));
        }
        ...
        {
            return (r.x < (x + width)) && (x < (r.x + r.width)) && (r.y < (y + height)) && (y < (r.y
                + r.height));
        }

        bool Intersects(const RECT& rct) const
        {
            return (rct.left < (x + width)) && (x < rct.right) && (rct.top < (y + height)) && (y <
                rct.bottom);
        }
    ...
    struct Vector2 : XMFLOAT2
    {
        ...
        XM_CONSTEXPR Vector2(int _x, int _y) : XMFLOAT2(static_cast<float>(_x),
                                                        static_cast<float>(_y))
        {
        }

        XM_CONSTEXPR Vector2(UINT _x, UINT _y) : XMFLOAT2(static_cast<float>(_x),
                                                          static_cast<float>(_y))
        {
        }
        ...
        static void Barycentric(const Vector2& v1, const Vector2& v2, const Vector2& v3, float f,
        ...
        static void Transform(_In_reads_(count) const Vector2* varray, size_t count,
                              const Matrix& m, _Out_writes_(count) Vector2* resultArray);

        static void Transform(const Vector2& v, const Matrix& m, Vector4& result);
        static void Transform(_In_reads_(count) const Vector2* varray, size_t count,
                              const Matrix& m, _Out_writes_(count) Vector4* resultArray);
    ...
    struct Vector3 : XMFLOAT3
    {
        ...
        static void Barycentric(const Vector3& v1, const Vector3& v2, const Vector3& v3, float f,
        ...
        static void Transform(_In_reads_(count) const Vector3* varray, size_t count,
                              const Matrix& m, _Out_writes_(count) Vector3* resultArray);
    ...
    struct Vector4 : XMFLOAT4
    {
        ...
        Vector4& operator =(const Vector3 _vec3)
        {
            x = _vec3.x;
            y = _vec3.y;
            z = _vec3.z;
        ...
        static void Barycentric(const Vector4& v1, const Vector4& v2, const Vector4& v3, float f,
    ...
    struct Matrix : XMFLOAT4X4
    {
        ...
        static Matrix CreateOrthographic(float width, float height, float zNearPlane,
                                         float zFarPlane);
        static Matrix CreateOrthographicOffCenter(float left, float right, float bottom, float top,
                                                  float zNearPlane, float zFarPlane);

        static Matrix CreateLookAt(const Vector3& position, const Vector3& target,
                                   const Vector3& up);
        static Matrix CreateWorld(const Vector3& position, const Vector3& forward,
                                  const Vector3& up);
...
{
    template <>
    struct less<SimpleMath::Rectangle>
    {
        bool operator()(const SimpleMath::Rectangle& r1, const SimpleMath::Rectangle& r2) const
        {
            return ((r1.x < r2.x)
                ...
                || ((r1.x == r2.x) && (r1.y == r2.y) && (r1.width == r2.width) && (r1.height < r2.
                    height)));
        }
    };

    template <>
    struct less<SimpleMath::Vector2>
    {
    ...
    template <>
    struct less<SimpleMath::Vector3>
    {
    ...
    template <>
    struct less<SimpleMath::Vector4>
    {
    ...
    template <>
    struct less<SimpleMath::Quaternion>
    {
    ...
    template <>
    struct less<SimpleMath::Viewport>
    {
